...
### 3 Jul afternoon — GRID TRUTH: the cynniam case → platform-wide display fix
- **Trigger:** Steve challenged cynniam's (user 157) grid page claiming "Grid 1 ✓ complete 36/36, $180 bonus paid". Investigation (fulfillment → ledger → gateway-forensics, canonical order): her **$140 of payments are REAL** (verified live at NOWPayments — 11 May: T1 $20, T2 $50, Nexus 100cr $20, 250cr $50; the identical 9-Jun DB timestamps are legitimate post-breach backfill). Her team's real activity: **~4 seats** (direct $8 = 40% old-plan ✓, uni $1.25 = 6.25% ✓, $4 = 5% pool × 4 × $20 ✓). The $72/$180 the UI showed = reversed phantom topups.
- **Audit** (`/admin/api/grid-truth-audit`, `dd38e502` + `613b4329` + `?summary=1`/`?top=N` `04f86a3d`): 84 complete grids = **4 genuine / 80 RETIRED / 0 unbacked**. `retired_at` was designed (12-Jun cutover) to distinguish retired-vs-genuine "for display" and the visualiser NEVER implemented it — 80 members saw "completed, bonus paid" for **$13,948 of reversed fiction**. Only paid-topup leak: **$18 on the house account** — zero members held phantom money.
- **Fix** (`ebf36ad3`): retired grids render honestly (neutral "Grid N · retired" chips, "retired at the plan change" headline, earnings-preserved copy); Advances tile counts genuine only; genuine completions untouched; commissions panel was always ledger-truth so no member's numbers moved. **Quiet correction, no broadcast** (Steve's call); support one-liner: the display showed a legacy-plan projection, real earnings never changed.
- **$18 reversed** (`/admin/api/reverse-house-topup`, dry-run→apply): row 589, house campaign wallet 257.73→239.73. Re-audit: **phantom_paid_topup_usd 0.0** — ledger perfectly clean platform-wide (13,966 reversed total, arithmetic self-verified).
...
- b69b002 FIXED gifted-pack draft activation. Root cause: a member's draft ad is activated only by the PURCHASE settlement (create_intent links+sizes, confirm flips draft->active); a GIFTED pack (grant-pack) writes the pack row directly and skips that flow, leaving gifted members' ads stuck draft/unlinked (share_approved=false, views_target=0) — owned pack but invisible, zero views. Added `_al_activate_pending_drafts()` (links draft->owned pack slot, activates, sizes from pack; reuses multi-video slot + aggregate-completion model) + `/admin/api/al/activate-drafts?username=X|?sweep=1(&dryrun=1)`; grant-pack now auto-activates the draft so it can't recur. Swept the affected cohort live: 5 members (worksmarter/itsamazing/gdpassive/kestola/Dreamlife), each 1 draft -> their $100 pack, target 8000; verified worksmarter in live rotation pool. (5 Aug)

- e793992 CONSOLIDATED to ONE Performance page. The standalone `/pack-performance` page (shipped 94e1ae3) duplicated the existing rich Campaign Analytics page (`/campaign-analytics`, "Real views from real people": totals, unique viewers, top countries, hour heatmap, per-campaign detail). Retired the standalone page (component/route/shell/nav removed); folded a **Pack progress** section (aggregate views vs each pack's total, % to completion, slots used) into Campaign Analytics, fed by the retained `/api/al/pack-performance` endpoint (renders only when packs exist, so pack-less admin accounts are unaffected). AL sidebar now has a single **Performance** item → `/campaign-analytics`. Also fixed the days-to-go bug (en.json `daysToGo` used `{{count}}` but the code passes `d`). Note: Campaign Analytics is SAP-token/Sora-styled (renders navy via remapped tokens) — inherited tech debt, not reworked. (4 Aug)
- 6ef274d MULTI-VIDEO PACKS (tiered, expiry-preserving): each pack now backs 2 video ads ($10/$20/$50), 4 ($100/$200/$400) or 6 ($600/$800/$1000) via `VIDEOS_BY_PACK` / `videos_allowed_for_level()`. Every video accrues its OWN views; the pack completes on the **aggregate** of its videos reaching the pack's UNCHANGED total target, so completion/expiry speed is identical to a single-video pack for ANY number of videos (1..N). New `video_campaigns.pack_purchase_id` link column accessed **only via raw SQL** and ensured at startup (before any request, works under SKIP_MIGRATIONS) so no existing ORM query is touched. Legacy single-video packs fall back to the `campaign_id` link and behave exactly as before. Create-gate allowance = sum of slots across owned packs. Verified: boot clean, no regression on campaign endpoints, tier helper unit-tested. NOT yet verified live: real create-2-videos + watch-to-aggregate-completion (needs a member session). Known cosmetic follow-up: per-video progress bars read against the pack's full total, so a multi-video pack shows each video partially filled — display-only, mechanic is correct. (4 Aug)
- 05bb9f2 Dashboard tier badge in the earnings hero: gold LIFETIME MEMBER / silver ANNUAL MEMBER, reading `is_lifetime`/`is_annual` from the membership-status call the dashboard already makes; free members see nothing. (4 Aug)
