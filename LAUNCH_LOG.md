...
- 94e1ae3 PACK PERFORMANCE page (`/pack-performance`, AlShell nav item after My Campaigns): per-pack card showing the **aggregate** views vs the pack's total target (the completion driver) with % to completion, slots used/total, and every video's own live view count with relative mini-bars; free-slot "add another video ad" nudge. Backed by `/api/al/pack-performance` (same pack↔video resolution as completion: `pack_purchase_id` new / `campaign_id` legacy). Resolves the multi-video analytics gap AND the per-video progress-bar confusion in one surface. Verified live: shell 200, chunk served, API 401-guarded. NOT yet eyeballed logged-in (needs a member session with packs). (4 Aug)
