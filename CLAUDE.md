...
**Member autoresponder direction (decided 29 Jun 2026, not yet built):** per-member domains — each member verifies their OWN sending domain (full reputation isolation; one bad sender only damages their own domain, never the platform's). Build by extending the existing `custom_domains.py` DNS-verification pattern (already proven for SuperPages landing pages) from CNAME→Railway to DKIM/SPF/DMARC→SES `VerifyDomainIdentity`. Sending layer is ~80% present already: `LeadList`/`MemberLead`/`EmailSequence`/`EmailSendLog` models + `/api/leads/*` surface (lists, upload-csv, sequences, broadcast, stats) + `ses_send` + SNS bounce webhook. Per-member `from_email` hook already exists (`MEMBER_FROM_EMAIL`, referenced ~46708/52268/52339). The build is the domain-verification onboarding + wiring verified member domains into `ses_send` — NOT a from-scratch email platform.
