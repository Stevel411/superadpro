...
## 📧 Email System — SINGLE SOURCE OF TRUTH (29 Jun 2026)

**Provider is chosen by the `EMAIL_PROVIDER` env var. The code only knows two values: `brevo` (DEFAULT if unset) and `ses`.** Resend is NOT in the email code path — a `RESEND_API_KEY` may exist in Railway as dead leftover; ignore it. Selector lives in `app/mailer.py::provider()` → `os.getenv("EMAIL_PROVIDER", "brevo")`. There is a SEPARATE `MEMBER_BULK_PROVIDER` (also brevo-default) for member bulk/autoresponder sends, so member campaigns can run on a different SES account than transactional mail.

- **LIVE STATE (29 Jun 2026): `EMAIL_PROVIDER=ses` and `MEMBER_BULK_PROVIDER=ses` are set in Railway. Confirmed sending + delivering via Amazon SES** (verified end-to-end with `/admin/api/test-email`). Before this, `EMAIL_PROVIDER` did NOT exist in Railway at all, so the platform silently defaulted to Brevo for weeks despite the SES migration being done — the migration was sitting behind a missing variable. If transactional mail ever goes silent again, FIRST check `EMAIL_PROVIDER` still exists and equals `ses`.
- SES creds in Railway: `SES_SMTP_HOST/PORT/USER/PASS`. Sending identity: `noreply@superadpro.com` (FROM_EMAIL). SES region must match the creds' region and the identity must be domain-verified + out of sandbox there.
- **Verify tool:** `GET /admin/api/test-email?to=<addr>` (session-admin-gated) — sends the real free-signup welcome and returns JSON {provider, member_bulk_provider, from_email, ses_smtp_host_set, sent}. Use this to confirm provider + delivery in one tap. (The old secret-gated `/admin/test-email` was deleted in security cleanup `3acdca1a0`; this is its session-auth replacement.)

**Welcome-email wiring (the 0%-activation fix, 29 Jun 2026):** there are TWO registration handlers and they behaved differently. `api_register` (line ~37494) always sent a welcome; `register_process` (line ~6511, the LIVE free-signup path) sent the new member NOTHING — only notified the sponsor. Free signups got silence, a direct contributor to 0% free→paid activation. Fixed: `register_process` now calls `send_welcome_free_email` after `db.commit()` (try/except, can't break signup). `send_welcome_free_email` (`app/email_utils.py`) is DISTINCT from `send_welcome_email` — the latter claims "Active" and lists is_pro-gated tools, which is FALSE for a free signup; the free version says "Free — tools locked", lists what $20/mo Partner unlocks, and CTAs to `/pay-membership`. Commits: `923b06a04` (wiring), `cae3b9c4b` (test endpoint).

**Member autoresponder — BUILT & PROVEN LIVE (29 Jun 2026).** Per-member domains: each member verifies their OWN sending domain in our SES account (full reputation isolation). Proven end-to-end against a real domain — see the "Most Recent Session (29 Jun 2026)" block below for the full architecture, the AWS IAM setup, the critical send-path fix (`f7ae1adc4`), and the `mail.directmailpro.net` inbox-delivery proof. Built by extending the `custom_domains.py` DNS-verification pattern to DKIM/SPF/DMARC→SES. (Historical context, now resolved: the sending layer was ~80% present — `LeadList`/`MemberLead`/`EmailSequence`/`EmailSendLog` + `/api/leads/*` + `ses_send` + SNS bounce webhook.)

## 🎯 Most Recent Session (29 Jun 2026) — Autoresponder proven live + email economics + watch-access fixes

Long session. Three big workstreams, all shipped and verified on production.

### 1. Per-member-domain autoresponder — BUILT & PROVEN END-TO-END (the headline)
Members verify their OWN sending domain inside our single SES account, so autoresponder mail sends from THEIR brand (subscribers never see SuperAdPro) and each domain carries its own reputation. **This is the supersession of the "not yet built" note in the Email block above — it is now built, live, and proven against a real domain.**

- **Architecture:** `SendingDomain` model (`app/database.py`, after CustomDomain) → `app/sending_domains.py` SES engine (`provision_domain` = `VerifyDomainDkim` → 3 DKIM CNAMEs + SPF + DMARC; `verify_one` polls `GetIdentityDkimAttributes`; `verify_all_pending` cron; `delete_domain`) → member API (`GET/POST /api/sending-domains`, `POST /api/sending-domains/{id}/verify`, `DELETE`, `GET /cron/verify-sending-domains`) → React `SendingDomains.jsx` (`/sending-domains`) + `HelpSendingDomain.jsx` (`/help/sending-domain`, per-provider DNS guides + "get a domain in 5 min") → discoverability banner on `/pro/leads`.
- **AWS IAM (only Steve can do, NOW DONE):** IAM user `superadpro-ses-api` + customer-managed policy `SuperAdPro-SES-DomainVerify` (ses:VerifyDomainIdentity/VerifyDomainDkim/GetIdentityVerificationAttributes/GetIdentityDkimAttributes/DeleteIdentity). Railway vars: `AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`, `AWS_SES_REGION=eu-west-1`. These are API creds — DIFFERENT from the `SES_SMTP_*` sending creds. Confirm via `/admin/api/setup-sending-domains` → `ses_api_configured:true`.
- **PROVEN:** real domain `mail.directmailpro.net` (GoDaddy) → 5 real DNS records generated by SES → added (GoDaddy doubles the domain in the Name field; pick the single-`.net` option) → auto-verified → **a real broadcast delivered to the Gmail INBOX from `Steve <steve@mail.directmailpro.net>`, DKIM-signed by the member domain.** Whole pipeline confirmed working.
- **THE CRITICAL SEND-PATH FIX (`f7ae1adc4`):** verification alone was decorative — all three member send paths (`_run_broadcast_job`, `api_send_sequence_email`, `_send_sequence_email`) read a single unset `MEMBER_FROM_EMAIL` env and sent as `noreply@superadpro.com`. New helper `_member_sender_identity(db, user_id)` resolves the member's most-recently-verified `SendingDomain` → `(from_email, from_name)`, wired into all three (sets From address AND name; falls back to platform sender when no verified domain). Found by an actual send test — verification "verified" but the email arrived from SuperAdPro. **Lesson reinforced: send a real email; "verified" ≠ "sends from the right place."**
- Commits: `6febe0722` (foundation+model+engine+table endpoint), `5158d26d4` (member API+cron), `529c684a0` (onboarding UI), `6bcf41bde` (nav entry), `ba2571da6` (get-a-domain guide), `f7ae1adc4` (send-from-member-domain fix).

### 2. Email allowance → 5,000/MONTH + boost packs −25%
- Free allowance moved from **100/DAY → 5,000/MONTH** (`MONTHLY_EMAIL_LIMIT=5000`). MONTHLY (not daily) is non-negotiable: a daily cap below list size means a member literally can't broadcast to their own 2-3k list in one send. New columns `emails_sent_month`/`emails_sent_month_key`; `_check_email_allowance`/`_deduct_email_send` use monthly free → then boost credits; `/api/leads/email-stats` + AllowanceBar show "Free this month X / 5,000".
- Boost packs cut 25%: **$3.75 / $14.25 / $21.75 / $74.25** (1k/5k/10k/50k). SES cost ≈ $0.10/1,000, so 5k free ≈ $0.50/member (5% of the $10 Partner margin); packs stay ~95%+ margin. Decided against generosity beyond this given $10/Partner, $5/Founder margins.
- Commits: `0fa2e7606` (step 1: price cut + migration endpoint), `afd35c09c` (step 2: monthly logic + UI).
- **⚠️ HARD-WON PATTERN — adding columns to `users` under SKIP_MIGRATIONS:** the `users` table is SELECTed on EVERY request by `get_current_user`. Declaring a model column the prod DB lacks breaks every users query (incl. the migration endpoint itself — chicken/egg). MUST be TWO deploys: (1) one-shot raw-SQL endpoint (`/admin/api/setup-monthly-email-allowance`) creates the columns, tap it; (2) THEN deploy the model-column declaration + logic. Did exactly this; both live.

### 3. Watch-to-Earn access — Gerard case + tier-0 Launchpad fix (`fc11aa1a2`)
- **Watch-to-Earn is gated on `get_user_highest_tier(db, uid) >= 0`** = an active (incomplete) owned `Grid`. Founding/Partner MEMBERSHIP ≠ a Campaign Tier; no grid → `-1` → locked. Grid tiers: 0=Launchpad($10),1=Starter($20)…8. Admin=8.
- **TIER-0 LAUNCHPAD BUG (fixed):** frontend lock was `!(highest_tier && highest_tier > 0)`. For tier 0, `0 && …` short-circuits falsy → Launchpad members wrongly locked though backend allows `>=0`. A naive `>0`→`>=0` swap does NOT fix it (same short-circuit). Fixed with explicit `typeof ht==='number' && ht>=0`. Also added a "Try again" retry button to the "Unable to load" dead-end.
- **Gerard (user 219, @musketeers):** account healthy throughout — paid $20 Starter via Stripe (`ch_3TbkaOBxEFGz0qoH11qI0R6r`), valid live tier-1 grid (256), passes every gate. His "Unable to load" was a TRANSIENT `/api/watch` 500 that cleared on its own; not his account, not the tier gate. The two reversed `grid_completion_bonus_topup` rows were the known 12-Jun erroneous-migration phantoms, not real completions.
- **🔑 KEY DIAGNOSTIC LEARNING:** `/admin/api/user-fulfillment` is BLIND to Stripe charges (`recorded_payment_count:0` falsely implied "never paid"). For the canonical Stripe truth use **`/admin/api/gateway-forensics?user_ids=N`** — it hits Stripe live (`Customer.list(email)→Charge.list`) + NOWPayments sweep. This is the go-to for any "did they actually pay?" dispute. Kept a live-error capture hook: `/admin/api/watch-last-error?user_id=N` (member loads watch → triggers real error → admin reads exact traceback) for if it recurs.

### Behavioural note (this session)
Chased two wrong theories on Gerard (wrongly-completed grid; then a send-write hypothesis) before the data disproved each. Pattern to keep: capture the real error/state before theorising; the read-only replay passing while the live path failed was the tell that the crash was in the write/live path. Also corrected own over-catastrophising on the autoresponder ("not buildable") — deliverability is rented from SES, the app layer was ~80% there.

### Outstanding / next session (priority order)
1. **5 CRITICAL health scanners from session start — STILL UNTOUCHED** (commission_routing, matrix_tree, pack_ownership, commission_cap "29 drifts", R2_storage). These are about LIVE MONEY and outrank features. Revisit first.
2. **Per-member sending caps** — the one real guardrail before promoting the autoresponder hard. 5k/month free is the soft cap; there's no hard per-member abuse cap on a shared SES account. Open Q: sane monthly hard cap (~10k suggested for 2-3k lists).
3. **Bounce/complaint auto-pause (SES SNS)** — may already exist (`8c1633f93` "Email hygiene part 2"); verify before building.
4. Email BODY still shows "SuperAdPro" branding inside the wrapper (sender/From is correct now; body footer not). Minor polish for full own-brand.
5. i18n batch still pending (pricing keys + training locales).

## 🎯 Previous Session (24 May 2026) — Matt recovery + BSC scanner Layers 4-6 + Stripe gap fix
