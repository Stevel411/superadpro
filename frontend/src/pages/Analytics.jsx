...
var TL={direct_sponsor:'Direct 30%',uni_level:'Uni-Level',grid_completion_bonus:'Grid Bonus',membership:'Membership',membership_renewal:'Renewal',membership_sponsor:'Membership',course_direct_sale:'Course Sale',course_pass_up:'Course Pass-Up','Membership Sponsor':'Membership',nexus_sponsor:'Creator Credits',nexus_level:'Creator Credits',nexus_completion:'Creator Credits',matrix_level:'Creator Credits',matrix_completion:'Creator Credits',direct_referral:'Creator Credits'};
...
function GC({gradient,label,value,sub,children}){
  return <div style={{background:gradient,borderRadius:14,padding:'16px 18px',position:'relative',overflow:'hidden'}}>
    ...
    <div style={{fontSize:13,color:'rgba(255,255,255,0.7)',fontWeight:600,marginBottom:4}}>{label}</div>
    <div style={{fontFamily:"'Sora',sans-serif",fontSize:22,fontWeight:900,color:'#fff'}}>{value}</div>
    {sub&&<div style={{fontSize:13,color:'rgba(255,255,255,0.5)',marginTop:3}}>{sub}</div>}
    {children}
  </div>;
}
...
function WC({title,subtitle,children,style}){
  return <div style={{background:'#fff',borderRadius:14,padding:'20px 22px',border:'1px solid #e2e8f0',boxShadow:'0 1px 3px rgba(0,0,0,0.04)',position:'relative',overflow:'hidden',...style}}>
    ...
    {subtitle&&<div style={{fontSize:12,color:'#7a8899',marginBottom:16}}>{subtitle}</div>}
    {children}
  </div>;
}
...
var cTick={color:'#7a8899',font:{size:11}};
...
export default function AnalyticsPage(){
  ...
  var breakdownRef=useChart(data?(function(){var b=data.income_breakdown,tot=b.grid+b.membership+b.courses+(b.nexus||0);var cols=['#22c55e','#f59e0b','#22d3ee','#38bdf8'];if(tot===0)return{type:'doughnut',data:{labels:[t('analytics.chart.noData')],datasets:[{data:[1],backgroundColor:['#e2e8f0'],borderWidth:0}]},options:{responsive:true,maintainAspectRatio:false,cutout:'72%',plugins:{legend:{display:false}}}};return{type:'doughnut',data:{labels:[t('analytics.chart.membership'),t('analytics.chart.grid'),t('analytics.chart.creditNexus'),t('analytics.chart.courses')],datasets:[{data:[b.membership,b.grid,b.nexus||0,b.courses],backgroundColor:cols,borderWidth:0,hoverOffset:10,borderRadius:4}]},options:{responsive:true,maintainAspectRatio:false,cutout:'72%',plugins:{legend:{position:'bottom',labels:{padding:14,usePointStyle:true,pointStyle:'circle',color:'#475569',font:{size:11,weight:600}}},tooltip:{...cTip,callbacks:{label:function(c){return c.label+': $'+c.parsed.toFixed(2)}}}}}};})():null);

  var gridRef=useChart(data&&data.grid_progress.length>0?{type:'bar',data:{labels:data.grid_progress.map(function(g){return 'T'+g.tier}),datasets:[{label:t('analytics.chart.filled'),data:data.grid_progress.map(function(g){return g.filled}),backgroundColor:['#22c55e','#0ea5e9','#1e3a8a','#0891b2','#f59e0b','#f97316','#ef4444','#ec4899'],borderRadius:6,barThickness:20},{label:t('analytics.chart.remaining'),data:data.grid_progress.map(function(g){return 16-g.filled}),backgroundColor:'#f1f5f9',borderRadius:6,barThickness:20}]},options:{indexAxis:'y',responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false},tooltip:{...cTip,callbacks:{label:function(c){return c.dataset.label+': '+c.parsed.x+'/16'}}}},scales:{x:{stacked:true,max:16,grid:cGrid,ticks:{...cTick,callback:function(v){return v}}},y:{stacked:true,grid:{display:false},ticks:cTick}}}}:null);

  var teamRef=useChart(data?{type:'bar',data:{labels:data.team_weekly.map(function(w){return 'W'+w.week}),datasets:[{label:t('analytics.chart.new'),data:data.team_weekly.map(function(w){return w.count}),backgroundColor:'#22c55e',borderRadius:6,barThickness:14}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false},tooltip:cTip},scales:{x:{grid:{display:false},ticks:cTick},y:{grid:cGrid,beginAtZero:true,ticks:cTick}}}}:null);

  var streamRef=useChart(data&&data.monthly_streams.length>0?{type:'bar',data:{labels:data.monthly_streams.map(function(m){return m.month}),datasets:[{label:t('analytics.chart.membership'),data:data.monthly_streams.map(function(m){return m.membership}),backgroundColor:'#22c55e',borderRadius:4},{label:t('analytics.chart.grid'),data:data.monthly_streams.map(function(m){return m.grid}),backgroundColor:'#f59e0b',borderRadius:4},{label:t('analytics.chart.nexus'),data:data.monthly_streams.map(function(m){return m.nexus||0}),backgroundColor:'#22d3ee',borderRadius:4},{label:t('analytics.chart.courses'),data:data.monthly_streams.map(function(m){return m.courses}),backgroundColor:'#38bdf8',borderRadius:4}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{position:'bottom',labels:{padding:12,usePointStyle:true,pointStyle:'circle',color:'#475569',font:{size:11,weight:600}}},tooltip:{...cTip,callbacks:{label:function(c){return c.dataset.label+': $'+c.parsed.y.toFixed(2)}}}},scales:{x:{stacked:true,grid:{display:false},ticks:cTick},y:{stacked:true,grid:cGrid,ticks:{...cTick,callback:function(v){return '$'+v}}}}}}:null);

  var watchRef=useChart(data&&data.daily_watches?{type:'bar',data:{labels:data.daily_watches.map(function(d){var p=d.date.split('-');return p[1]+'/'+p[2]}),datasets:[{label:t('analytics.chart.watched'),data:data.daily_watches.map(function(d){return d.count}),backgroundColor:'#22d3ee',borderRadius:4,barThickness:8}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false},tooltip:{...cTip,callbacks:{label:function(c){return c.parsed.y+' '+t('analytics.chart.videos')}}}},scales:{x:{grid:{display:false},ticks:{...cTick,maxTicksLimit:10}},y:{grid:cGrid,beginAtZero:true,ticks:{...cTick,stepSize:1}}}}}:null);

  if(loading)return<AppLayout title={t('campaignAnalytics.analyticsTitle')}><div style={{display:'flex',alignItems:'center',justifyContent:'center',padding:80,minHeight:'60vh'}}><div style={{display:'flex',flexDirection:'column',alignItems:'center',gap:16}}><div style={{width:44,height:44,borderRadius:'50%',border:'3px solid #e2e8f0',borderTopColor:'#0ea5e9',animation:'spin 0.8s linear infinite'}}/><div style={{fontFamily:"'Sora',sans-serif",fontSize:14,fontWeight:700,color:'#7a8899'}}>{t('campaignAnalytics.loadingAnalytics')}</div></div><style>{'@keyframes spin{to{transform:rotate(360deg)}}'}</style></div></AppLayout>;
  ...
  var empty=function(msg){return<div style={{height:200,display:'flex',alignItems:'center',justifyContent:'center',color:'#7a8899',fontSize:13,fontWeight:600}}>{msg}</div>};
  ...
  return(
    ...
    <div style={{padding:'28px 24px 60px'}}>
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:12,marginBottom:16}}>
        ...
        <GC gradient="linear-gradient(135deg,#b45309,#f59e0b)" label={t('analytics.campaignGrid',{defaultValue:'Campaign Grid'})} value={'$'+(tot.grid_earnings||0).toFixed(2)} sub={t('analytics.gridCommissions',{defaultValue:'Grid commissions'})}>
          <div style={{position:'absolute',top:14,right:14}}><Spark data={mSpark('grid')} color="#fff"/></div>
        </GC>
        <GC gradient="linear-gradient(135deg,#1e3a8a,#0ea5e9)" label={t('analytics.creditNexus',{defaultValue:'Creator Credits'})} value={'$'+(tot.nexus_earnings||0).toFixed(2)} sub={t('analytics.nexusCommissions',{defaultValue:'Flat 20% on credit-pack referrals'})}>
      ...
      <div style={{display:'grid',gridTemplateColumns:'3fr 2fr',gap:12,marginBottom:12}}>
        <WC title={t('analytics.networkByCountry')} subtitle={t('analytics.whereTeamBased')}>
          <div style={{borderRadius:10,overflow:'hidden',background:'#eef2ff',border:'1px solid #e2e8f0',padding:4}}>
            <WorldMap
              ...
              styleFunction={function(ctx){
                ...
                return{
                  fill:hasValue?'#1d4ed8':'#7a8899',
                  fillOpacity:hasValue?0.9:0.3,
                  stroke:hasValue?'#1e40af':'#475569',
          ...
          {nc.length>0&&<div style={{display:'flex',gap:12,marginTop:12,flexWrap:'wrap'}}>
            {nc.map(function(c,i){return<div key={i} style={{display:'flex',alignItems:'center',gap:6}}><div style={{width:10,height:10,borderRadius:'50%',background:'linear-gradient(135deg,#2563eb,#3b82f6)'}}/><span style={{fontSize:12,fontWeight:600,color:'#334155'}}>{c.code}</span><span style={{fontSize:13,padding:'2px 8px',borderRadius:10,background:'#f1f5f9',color:'#475569'}}>{c.count}</span></div>})}
          </div>}
          {nc.length===0&&<div style={{textAlign:'center',padding:'12px 0',color:'#7a8899',fontSize:12}}>{t('analytics.mapEmpty')}</div>}
        </WC>
      ...
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:12,marginBottom:12}}>
        <WC title={t('analytics.linkPerformance')} subtitle={t('analytics.yourLinks')}>
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10,marginBottom:12}}>
            <div style={{background:'#f0fdf4',borderRadius:10,padding:'12px 14px'}}><div style={{fontSize:13,fontWeight:700,color:'#15803d',textTransform:'uppercase',letterSpacing:1,marginBottom:3}}>{t('analytics.totalClicks')}</div><div style={{fontFamily:"'Sora',sans-serif",fontSize:22,fontWeight:900,color:'#166534'}}>{ls.total_clicks||0}</div></div>
            <div style={{background:'#eff6ff',borderRadius:10,padding:'12px 14px'}}><div style={{fontSize:13,fontWeight:700,color:'#1d4ed8',textTransform:'uppercase',letterSpacing:1,marginBottom:3}}>{t('analytics.last7Days')}</div><div style={{fontFamily:"'Sora',sans-serif",fontSize:22,fontWeight:900,color:'#1e3a8a'}}>{ls.clicks_7d||0}</div></div>
          </div>
          {ls.devices&&(ls.devices.mobile||ls.devices.desktop)?<div style={{marginBottom:10}}><div style={{fontSize:13,fontWeight:700,color:'#7a8899',marginBottom:6}}>{t('analytics.deviceSplit')}</div><div style={{display:'flex',height:8,borderRadius:4,overflow:'hidden',background:'#f1f5f9'}}>{ls.devices.mobile&&<div style={{width:(ls.devices.mobile/(ls.total_clicks||1)*100)+'%',background:'#22c55e',borderRadius:4}}/>}{ls.devices.desktop&&<div style={{width:(ls.devices.desktop/(ls.total_clicks||1)*100)+'%',background:'#3b82f6',borderRadius:4}}/>}</div><div style={{display:'flex',gap:12,marginTop:6}}>{ls.devices.mobile&&<span style={{fontSize:13,color:'#475569'}}>📱 Mobile: {ls.devices.mobile}</span>}{ls.devices.desktop&&<span style={{fontSize:13,color:'#475569'}}>🖥 Desktop: {ls.devices.desktop}</span>}</div></div>:null}
          {(ls.top_countries||[]).length>0&&<div><div style={{fontSize:13,fontWeight:700,color:'#7a8899',marginBottom:6}}>{t('analytics.topCountries')}</div>{ls.top_countries.map(function(c,i){return<div key={i} style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'4px 0'}}><span style={{fontSize:12,color:'#334155'}}>{c.country}</span><span style={{fontSize:12,fontWeight:700,color:'#0f172a'}}>{c.clicks}</span></div>})}</div>}
          {!ls.total_clicks&&<div style={{textAlign:'center',padding:'16px 0',color:'#7a8899',fontSize:12}}>{t('analytics.shareLink')}</div>}
        </WC>

        <WC title={t('analytics.withdrawals')} subtitle={t('analytics.usdtPolygon')}>
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10,marginBottom:12}}>
            <div style={{background:'#ecfeff',borderRadius:10,padding:'12px 14px'}}><div style={{fontSize:13,fontWeight:700,color:'#0e7490',textTransform:'uppercase',letterSpacing:1,marginBottom:3}}>{t('analytics.totalWithdrawn')}</div><div style={{fontFamily:"'Sora',sans-serif",fontSize:22,fontWeight:900,color:'#0a1438'}}>${ws.total_withdrawn||'0.00'}</div></div>
            <div style={{background:'#fffbeb',borderRadius:10,padding:'12px 14px'}}><div style={{fontSize:13,fontWeight:700,color:'#b45309',textTransform:'uppercase',letterSpacing:1,marginBottom:3}}>{t('analytics.pending')}</div><div style={{fontFamily:"'Sora',sans-serif",fontSize:22,fontWeight:900,color:'#92400e'}}>${ws.pending||'0.00'}</div></div>
          </div>
          {ws.last_date?<div style={{background:'#f8fafc',borderRadius:10,padding:'12px 14px'}}><div style={{fontSize:13,color:'#7a8899',marginBottom:4}}>{t('analytics.lastWithdrawal')}</div><div style={{display:'flex',justifyContent:'space-between'}}><span style={{fontSize:13,fontWeight:700,color:'#0f172a'}}>${ws.last_amount}</span><span style={{fontSize:12,color:'#475569'}}>{ws.last_date}</span></div></div>:<div style={{textAlign:'center',padding:'16px 0',color:'#7a8899',fontSize:12}}>{t('analytics.noWithdrawals')}</div>}
        </WC>

        <WC title={t('analytics.aiToolsUsage')} subtitle={t('analytics.lifetimeGenerated')}>
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10}}>
            {[{label:'Campaign Studio',val:ai.campaign_studio||0,bg:'#eff6ff',color:'#1d4ed8'},{label:'Social Posts',val:ai.social_posts||0,bg:'#f0fdf4',color:'#15803d'},{label:'Video Scripts',val:ai.video_scripts||0,bg:'#ecfeff',color:'#0e7490'},{label:'Niche Finder',val:ai.niche_finder||0,bg:'#fffbeb',color:'#b45309'}].map(function(a,i){return<div key={i} style={{background:a.bg,borderRadius:10,padding:'12px 14px'}}><div style={{fontSize:13,fontWeight:700,color:a.color,textTransform:'uppercase',letterSpacing:1,marginBottom:3}}>{a.label}</div><div style={{fontFamily:"'Sora',sans-serif",fontSize:22,fontWeight:900,color:a.color}}>{a.val}</div></div>})}
          </div>
      ...
      <WC title={t('campaignAnalytics.recentCommissions')}>
        {data.recent_commissions.length>0?<div style={{maxHeight:360,overflowY:'auto',overflowX:'auto',marginTop:8,borderRadius:8,border:'1px solid #f1f5f9'}}>
          <table style={{width:'100%',borderCollapse:'collapse',fontSize:13}}><thead style={{position:'sticky',top:0,background:'#fff',zIndex:1,boxShadow:'0 1px 0 #e2e8f0'}}><tr>
            ...
            {data.recent_commissions.map(function(c,i){var tl=TL[c.type]||c.type,tc=TC[c.type]||'#475569';return<tr key={i} style={{transition:'background 0.15s'}} onMouseEnter={function(e){e.currentTarget.style.background='#f8fafc'}} onMouseLeave={function(e){e.currentTarget.style.background='transparent'}}>
              ...
              <td style={{padding:'10px 12px',borderBottom:'1px solid #f1f5f9'}}><span style={{display:'inline-block',padding:'3px 10px',borderRadius:8,fontSize:13,fontWeight:800,textTransform:'uppercase',letterSpacing:0.5,background:tc+'15',color:tc}}>{tl}</span></td>
        ...
        </div>:<div style={{padding:'40px 0',textAlign:'center',color:'#7a8899',fontSize:13,fontWeight:600}}>{t('campaignAnalytics.noCommissions')}</div>}
      </WC>
