...
export default function AnalyticsPage() {
  ...
  if (loading) return <AppLayout title={t('campaignAnalytics.analyticsTitle')}><div style={{display:'flex',alignItems:'center',justifyContent:'center',padding:80}}><div style={{fontFamily:"'Sora',sans-serif",fontSize:16,fontWeight:700,color:'var(--sap-text-muted)'}}>{t('campaignAnalytics.loadingAnalytics')}</div></div></AppLayout>;
  if (error) return <AppLayout title={t('campaignAnalytics.analyticsTitle')}><div style={{display:'flex',alignItems:'center',justifyContent:'center',flexDirection:'column',gap:12,padding:80}}><div style={{fontFamily:"'Sora',sans-serif",fontSize:16,fontWeight:700,color:'var(--sap-red-bright)'}}>{t('campaignAnalytics.errorLoading')}</div><div style={{fontSize:14,color:'var(--sap-text-muted)'}}>{error}</div><a href="/login" style={{marginTop:8,padding:'10px 24px',borderRadius:10,background:'var(--sap-accent)',color:'#fff',textDecoration:'none',fontWeight:700,fontSize:14}}>{t('campaignAnalytics.signIn')}</a></div></AppLayout>;
