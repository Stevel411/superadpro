...
export default function AnalyticsPage(){
  ...
  var gridRef=useChart(data&&data.grid_progress.length>0?{type:'bar',data:{labels:data.grid_progress.map(function(g){return 'T'+g.tier}),datasets:[{label:t('analytics.chart.filled'),data:data.grid_progress.map(function(g){return g.filled}),backgroundColor:['#22c55e','#0ea5e9','#1e3a8a','#0891b2','#f59e0b','#f97316','#ef4444','#ec4899'],borderRadius:6,barThickness:20},{label:t('analytics.chart.remaining'),data:data.grid_progress.map(function(g){return 36-g.filled}),backgroundColor:'#f1f5f9',borderRadius:6,barThickness:20}]},options:{indexAxis:'y',responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false},tooltip:{...cTip,callbacks:{label:function(c){return c.dataset.label+': '+c.parsed.x+'/36'}}}},scales:{x:{stacked:true,max:36,grid:cGrid,ticks:{...cTick,callback:function(v){return v}}},y:{stacked:true,grid:{display:false},ticks:cTick}}}}:null);
