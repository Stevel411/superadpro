...
export default function AnalyticsPage(){
  ...
  var breakdownRef=useChart(data?(function(){var b=data.income_breakdown,tot=b.grid+b.membership+b.courses+(b.nexus||0);var cols=['#22c55e','#f59e0b','#a78bfa','#38bdf8'];if(tot===0)return{type:'doughnut',data:{labels:[t('analytics.chart.noData')],datasets:[{data:[1],backgroundColor:['#e2e8f0'],borderWidth:0}]},options:{responsive:true,maintainAspectRatio:false,cutout:'72%',plugins:{legend:{display:false}}}};return{type:'doughnut',data:{labels:[t('analytics.chart.membership'),t('analytics.chart.grid'),t('analytics.chart.creditNexus'),t('analytics.chart.courses')],datasets:[{data:[b.membership,b.grid,b.nexus||0,b.courses],backgroundColor:cols,borderWidth:0,hoverOffset:10,borderRadius:4}]},options:{responsive:true,maintainAspectRatio:false,cutout:'72%',plugins:{legend:{position:'bottom',labels:{padding:14,usePointStyle:true,pointStyle:'circle',color:'#475569',font:{size:11,weight:600}}},tooltip:{...cTip,callbacks:{label:function(c){return c.label+': $'+c.parsed.toFixed(2)}}}}}};})():null);

  var gridRef=useChart(data&&data.grid_progress.length>0?{type:'bar',data:{labels:data.grid_progress.map(function(g){return 'T'+g.tier}),datasets:[{label:t('analytics.chart.filled'),data:data.grid_progress.map(function(g){return g.filled}),backgroundColor:['#22c55e','#0ea5e9','#6366f1','#8b5cf6','#f59e0b','#f97316','#ef4444','#ec4899'],borderRadius:6,barThickness:20},{label:t('analytics.chart.remaining'),data:data.grid_progress.map(function(g){return 36-g.filled}),backgroundColor:'#f1f5f9',borderRadius:6,barThickness:20}]},options:{indexAxis:'y',responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false},tooltip:{...cTip,callbacks:{label:function(c){return c.dataset.label+': '+c.parsed.x+'/36'}}}},scales:{x:{stacked:true,max:36,grid:cGrid,ticks:{...cTick,callback:function(v){return v}}},y:{stacked:true,grid:{display:false},ticks:cTick}}}}:null);
  ...
  var streamRef=useChart(data&&data.monthly_streams.length>0?{type:'bar',data:{labels:data.monthly_streams.map(function(m){return m.month}),datasets:[{label:t('analytics.chart.membership'),data:data.monthly_streams.map(function(m){return m.membership}),backgroundColor:'#22c55e',borderRadius:4},{label:t('analytics.chart.grid'),data:data.monthly_streams.map(function(m){return m.grid}),backgroundColor:'#f59e0b',borderRadius:4},{label:t('analytics.chart.nexus'),data:data.monthly_streams.map(function(m){return m.nexus||0}),backgroundColor:'#a78bfa',borderRadius:4},{label:t('analytics.chart.courses'),data:data.monthly_streams.map(function(m){return m.courses}),backgroundColor:'#38bdf8',borderRadius:4}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{position:'bottom',labels:{padding:12,usePointStyle:true,pointStyle:'circle',color:'#475569',font:{size:11,weight:600}}},tooltip:{...cTip,callbacks:{label:function(c){return c.dataset.label+': $'+c.parsed.y.toFixed(2)}}}},scales:{x:{stacked:true,grid:{display:false},ticks:cTick},y:{stacked:true,grid:cGrid,ticks:{...cTick,callback:function(v){return '$'+v}}}}}}:null);
