...
export default function Academy() {
  ...
  useEffect(() => { apiGet('/api/al/academy').then(setData).catch(() => setData({ categories: [], courses: [] })); }, []);

  const courses = data ? data.courses : [];
  const cats = data ? ['All', ...data.categories] : ['All'];
  const shown = filter === 'All' ? courses : courses.filter(c => c.category === filter);
  const totalLessons = courses.reduce((n, c) => n + (c.lessons || 0), 0);
  ...
  return (
    <AlShell>
      ...
      <div className="acad">
        <div className="hero">
          <h1>AdvantageLife <b>Academy</b></h1>
          <p>The exact playbook to grow your business — hand-picked lessons from the world's top marketers, put in the right order. Free with your membership.</p>
          <div className="stats">
        ...
        {!data ? (
          ...
          <div className="empty">No courses here yet — check back soon.</div>
        ) : (
          <div className="grid">
            {shown.map(c => (
              <div key={c.slug} className="course" onClick={() => nav('/academy/' + c.slug)}>
                <div className="thumb" style={{ background: `linear-gradient(135deg,${c.cover_color || '#0a1f52'},${c.cover_color2 || '#12388f'})` }}>
                  <span className="cat">{c.category}</span>
                  <div className="play">▶</div>
                  <span className="dur">{c.lessons} lesson{c.lessons === 1 ? '' : 's'}</span>
                </div>
                <div className="cbody">
                  <h3>{c.title}</h3>
                  {c.description ? <div className="by">{(c.description || '').slice(0, 70)}{(c.description || '').length > 70 ? '…' : ''}</div> : null}
                  <div className="cmeta"><span className="lvl">{c.level}</span>{c.progress_pct > 0 ? <span>· {c.progress_pct}% done</span> : null}</div>
                  <div className="pbar"><i style={{ width: (c.progress_pct || 0) + '%' }} /></div>
                </div>
              </div>
            ))}
          </div>
