...
export default function PackPerformance() {
  ...
  if (loading) {
  ...
  } else if (!data || !data.packs || data.packs.length === 0) {
    body = (
    ...
    body = data.packs.map(function (p) {
        ...
        <div key={p.pack_id} style={CARD}>
          ...
          <div style={{ borderTop: '1px solid #eef1f8', paddingTop: 16 }}>
            ...
            {(p.videos || []).map(function (v, i) {
              ...
              return (
                <div key={v.id} style={{ display: 'flex', alignItems: 'center', gap: 13, marginBottom: 13 }}>
                  <div style={{ width: 30, height: 30, borderRadius: 9, background: NAVY, color: '#fff', fontWeight: 900, fontSize: 13, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>{i + 1}</div>
