      ...
      <section style={{...sec,background:T.grad2}}>
        <div style={{maxWidth:960,margin:'0 auto',textAlign:'center'}}>
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(2,1fr)',gap:24,maxWidth:680,margin:'48px auto 0'}}>
            <R><div style={{padding:'40px 28px',borderRadius:24,textAlign:'center',background:T.card,border:'1px solid '+T.cardBorder}}><div style={{fontFamily:"'Sora',sans-serif",fontSize:11,fontWeight:800,letterSpacing:2,textTransform:'uppercase',color:T.textDim,marginBottom:6}}>{t('superLink.basic')}</div><div style={{fontFamily:"'Sora',sans-serif",fontSize:52,fontWeight:900,color:T.heading,lineHeight:1}}>$20<span style={{fontSize:16,color:T.textDim,fontWeight:600}}>{t('superLink.perMonth')}</span></div><div style={{fontSize:13,color:T.textDim,margin:'6px 0 20px'}}>{t('superLink.allEssentialsFull')}</div>{['Your SuperLink','50% recurring commissions','8×8 Profit Grid','Campaign Studio','LinkHub + Link Tools','Course Academy','Training Centre','Social Share Suite'].map(function(f,i){return<div key={i} style={{fontSize:13,color:T.textMuted,padding:'7px 0',textAlign:'left',borderBottom:'1px solid '+T.featBorder}}><span style={{color:T.check,fontWeight:800}}>✓</span> {f}</div>})}</div></R>
