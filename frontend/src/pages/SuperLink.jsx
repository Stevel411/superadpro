...
var THEMES = {
  space: {
    name: 'Deep Space', bg: 'var(--sap-cobalt-deep)', bg2: 'var(--sap-cobalt-deep)', bg3: 'var(--sap-cobalt-deep)',
    text: 'var(--sap-border)', textMuted: 'var(--sap-text-faint)', textDim: 'var(--sap-text-muted)', textFaint: 'var(--sap-text-secondary)',
    ...
    footerBorder: 'rgba(255,255,255,0.03)', footerText: 'var(--sap-text-primary)', footerLink: '#334155',
    particles: ['99,102,241','14,165,233','139,92,246','16,185,129','251,191,36'],
    pLine: '99,102,241', pAlpha: 0.25, pLineAlpha: 0.06,
    gridBg: 'var(--sap-cobalt-deep)', gridText: '#fff', gridAccent: 'var(--sap-accent-light)',
    grad1: 'linear-gradient(180deg,#172554 0%,#172554 50%,#172554 100%)',
    grad2: 'linear-gradient(180deg,#172554 0%,#172554 50%,#172554 100%)',
    calloutBg: 'rgba(139,92,246,0.04)', calloutBorder: 'rgba(139,92,246,0.12)', calloutStrong: 'var(--sap-purple-light)',
    glow: 'radial-gradient(circle,rgba(251,191,36,0.06),transparent 60%)',
    basicBg: 'rgba(14,165,233,0.1)', basicC: 'var(--sap-accent-light)', basicBd: 'rgba(14,165,233,0.2)',
    proBg: 'rgba(139,92,246,0.1)', proC: 'var(--sap-purple-light)', proBd: 'rgba(139,92,246,0.2)',
    check: 'var(--sap-green-mid)', featBorder: 'rgba(255,255,255,0.04)',
  },
  gold: {
    ...
    gridBg: '#0f0f0f', gridText: '#fff', gridAccent: 'var(--sap-amber-bright)',
    grad1: 'linear-gradient(180deg,#0a0a0a 0%,#141414 50%,#0a0a0a 100%)',
    grad2: 'linear-gradient(180deg,#0a0a0a 0%,#111 50%,#0a0a0a 100%)',
    calloutBg: 'rgba(251,191,36,0.04)', calloutBorder: 'rgba(251,191,36,0.12)', calloutStrong: 'var(--sap-amber-bright)',
    glow: 'radial-gradient(circle,rgba(251,191,36,0.08),transparent 60%)',
    basicBg: 'rgba(251,191,36,0.08)', basicC: 'var(--sap-amber-bright)', basicBd: 'rgba(251,191,36,0.15)',
    proBg: 'rgba(251,191,36,0.15)', proC: '#fde68a', proBd: 'rgba(251,191,36,0.25)',
    check: 'var(--sap-amber-bright)', featBorder: 'rgba(251,191,36,0.06)',
  },
  light: {
    name: 'Clean Light', bg: 'var(--sap-bg-elevated)', bg2: '#ffffff', bg3: 'var(--sap-bg-page)',
    text: '#334155', textMuted: 'var(--sap-text-muted)', textDim: 'var(--sap-text-faint)', textFaint: 'var(--sap-text-ghost)',
    heading: 'var(--sap-text-primary)', strong: 'var(--sap-text-primary)',
    card: '#ffffff', cardBorder: 'var(--sap-border)', cardHover: 'var(--sap-text-ghost)',
    ...
    tierBg: '#ffffff', tierBorder: 'var(--sap-border)',
    ...
    popBadge: 'linear-gradient(135deg,#0ea5e9,#6366f1)',
    footerBorder: 'var(--sap-border)', footerText: 'var(--sap-text-faint)', footerLink: 'var(--sap-text-muted)',
    particles: ['14,165,233','99,102,241','139,92,246','16,185,129','245,158,11'],
    pLine: '99,102,241', pAlpha: 0.12, pLineAlpha: 0.04,
    gridBg: 'var(--sap-text-primary)', gridText: '#fff', gridAccent: 'var(--sap-accent-light)',
    ...
    proBg: 'rgba(99,102,241,0.08)', proC: 'var(--sap-indigo)', proBd: 'rgba(99,102,241,0.15)',
    check: 'var(--sap-green-mid)', featBorder: 'var(--sap-bg-page)',
  },
};
...
var TOOLS = [
  { name:'AI Content Creator', desc:'Generate social posts, email sequences, and ad copy with AI', badge:'PRO', color:'var(--sap-accent-light)', icon:'M13,2 L3,14 L12,14 L11,22 L21,10 L12,10Z' },
  ...
  { name:'Link Tools', desc:'Smart links, QR codes, click tracking, retargeting pixels', badge:'BASIC', color:'var(--sap-pink)', icon:'M15,3 L21,3 L21,9 M9,21 L3,21 L3,15 M21,3 L14,10 M3,21 L10,14' },
...
var STREAMS = [
  { name:'Membership Commissions', rate:'50% recurring', rc:'#34d399', desc:'Earn $10 to $29.50 per member every month — for as long as they stay active. Three tiers: Basic, Pro, and Creator.', ac:'var(--sap-green-mid)' },
  { name:'8×8 Profit Grid', rate:'Up to $7,200 per cycle', rc:'#818cf8', desc:'8 campaign tiers from $20 to $1,000. Your grid has 64 positions — each one pays you 6.25%. Grids auto-renew on completion.', ac:'var(--sap-indigo)' },
  { name:'Course Marketplace', rate:'100% commissions', rc:'var(--sap-amber-bright)', desc:"Keep 100% of every sale. Sales 2, 4, 6, 8 pass up — cascading infinitely deep. Income from people you've never met.", ac:'var(--sap-amber)' },
  { name:'SuperMarket', rate:'50 / 25 / 25', rc:'var(--sap-accent-light)', desc:'Sell digital products. Creators earn 50%, affiliates 25%. Create once, earn forever.', ac:'var(--sap-accent)' },
];
    ...
    <div style={{ background: T.bg, color: T.text, fontFamily: "'DM Sans',sans-serif", minHeight: '100vh' }}>
      ...
      <section style={{ ...sec, minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        ...
        <div style={{ position: 'relative', zIndex: 2, maxWidth: 820, textAlign: 'center' }}>
          ...
          <R delay={0.1}><h1 style={{ fontFamily:"'Sora',sans-serif", fontSize:'clamp(42px,8vw,84px)', fontWeight:900, lineHeight:0.92, color:T.heading, marginBottom:12 }}>Build Real<br/>{t('superLink.onlineIncome')}</h1></R>
          ...
          <R delay={0.3}><p style={{ fontSize:'clamp(15px,1.8vw,18px)', color:T.textMuted, maxWidth:620, margin:'0 auto 20px', lineHeight:1.7 }}>Your business = <strong style={{color:T.strong}}>{t('superLink.fourIncomeStreams')}</strong>. AI-powered tools. A Profit Grid that pays you up to <strong style={gold}>$22,824</strong> across all 8 campaign tiers. Recurring campaigns that never stop.</p></R>
      ...
      <section style={{...sec,background:T.grad1}}>
        <div style={{maxWidth:960,margin:'0 auto',textAlign:'center'}}>
          ...
          <R delay={0.2}><div style={{marginTop:60,borderRadius:24,overflow:'hidden',border:'1px solid '+T.cardBorder,background:T.gridBg}}><div style={{textAlign:'center',padding:'24px 20px 0'}}><div style={{fontFamily:"'Sora',sans-serif",fontSize:'clamp(18px,3vw,24px)',fontWeight:900,color:T.gridText}}>Watch Your <span style={{color:T.gridAccent}}>{t('superLink.profitGrid')}</span> Build</div><p style={{fontSize:13,color:'rgba(200,220,255,0.4)',marginTop:6}}>{t("superLink.selectTierDesc")}</p></div><div style={{height:380,padding:'8px 16px 16px'}}><Suspense fallback={<div style={{height:380,display:'flex',alignItems:'center',justifyContent:'center',color:T.gridAccent,fontFamily:"'Sora',sans-serif",fontSize:14,fontWeight:700}}>{t('common.loading3D')}</div>}><IncomeGrid3D height={360} showControls autoPlay /></Suspense></div></div></R>
        </div>
      </section>
      ...
      <section style={{...sec,background:T.grad2}}>
        <div style={{maxWidth:960,margin:'0 auto',textAlign:'center'}}>
          <R><h2 style={h2s}>{t("superLink.totalMarketingSuite")}</h2></R>
          <R delay={0.1}><p style={{fontSize:17,color:T.textDim,maxWidth:540,margin:'12px auto 0'}}>{t("superLink.marketingSuiteDesc")}</p></R>
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(2,1fr)',gap:24,maxWidth:680,margin:'48px auto 0'}}>
            <R><div style={{padding:'40px 28px',borderRadius:24,textAlign:'center',background:T.card,border:'1px solid '+T.cardBorder}}><div style={{fontFamily:"'Sora',sans-serif",fontSize:11,fontWeight:800,letterSpacing:2,textTransform:'uppercase',color:T.textDim,marginBottom:6}}>{t('superLink.basic')}</div><div style={{fontFamily:"'Sora',sans-serif",fontSize:52,fontWeight:900,color:T.heading,lineHeight:1}}>$20<span style={{fontSize:16,color:T.textDim,fontWeight:600}}>{t('superLink.perMonth')}</span></div><div style={{fontSize:13,color:T.textDim,margin:'6px 0 20px'}}>{t('superLink.allEssentialsFull')}</div>{['Your SuperLink','50% recurring commissions','8×8 Profit Grid','Campaign Studio','LinkHub + Link Tools','Course Marketplace','Training Centre','Social Share Suite'].map(function(f,i){return<div key={i} style={{fontSize:13,color:T.textMuted,padding:'7px 0',textAlign:'left',borderBottom:'1px solid '+T.featBorder}}><span style={{color:T.check,fontWeight:800}}>✓</span> {f}</div>})}</div></R>
            <R delay={0.1}><div style={{padding:'40px 28px',borderRadius:24,textAlign:'center',background:T.popBg,border:'1px solid '+T.popBorder,position:'relative'}}><div style={{position:'absolute',top:-1,left:'50%',transform:'translateX(-50%)',background:T.popBadge,color:'#fff',fontFamily:"'Sora',sans-serif",fontSize:9,fontWeight:800,letterSpacing:2,padding:'4px 16px',borderRadius:'0 0 8px 8px'}}>{t('superLink.popularLabel')}</div><div style={{fontFamily:"'Sora',sans-serif",fontSize:11,fontWeight:800,letterSpacing:2,textTransform:'uppercase',color:T.textDim,marginBottom:6}}>{t('superLink.pro')}</div><div style={{fontFamily:"'Sora',sans-serif",fontSize:52,fontWeight:900,color:T.heading,lineHeight:1}}>$35<span style={{fontSize:16,color:T.textDim,fontWeight:600}}>{t('superLink.perMonth')}</span></div><div style={{fontSize:13,color:T.textDim,margin:'6px 0 20px'}}>{t('superLink.fullAiAutomation')}</div>{['Everything in Basic','AI Content Creator','AI Funnel Generator','SuperPages builder','Email Autoresponder','Lead Dashboard','Priority support'].map(function(f,i){return<div key={i} style={{fontSize:13,color:T.textMuted,padding:'7px 0',textAlign:'left',borderBottom:'1px solid '+T.featBorder}}><span style={{color:T.check,fontWeight:800}}>✓</span> {f}</div>})}</div></R>
          </div>
        </div>
      </section>
      ...
      <section style={{...sec,textAlign:'center',padding:'80px 24px'}}>
        ...
        <div style={{maxWidth:960,margin:'0 auto',position:'relative'}}>
          <R><h2 style={{fontFamily:"'Sora',sans-serif",fontSize:'clamp(36px,6vw,56px)',fontWeight:900,color:T.heading,marginBottom:24}}>{sponsor} is building.<br/><span style={gold}>{t('superLink.nowYourTurn')}</span></h2></R>
          <R delay={0.15}><div><a href="/register" style={cta}>{t('superLink.createFreeAccount')}</a><div style={{fontSize:13,color:T.textFaint,marginTop:14}}>{t('superLink.freeNoCreditCard')}</div></div></R>
        </div>
      </section>

      <footer style={{position:'relative',zIndex:1,textAlign:'center',padding:'40px 24px',borderTop:'1px solid '+T.footerBorder}}>
        <p style={{fontSize:11,color:T.footerText,lineHeight:1.7}}>SuperAdPro · <a href="/legal" style={{color:T.footerLink,textDecoration:'none'}}>{t('superLink.terms')}</a> · <a href="/legal" style={{color:T.footerLink,textDecoration:'none'}}>{t('superLink.privacy')}</a> · <a href="/support" style={{color:T.footerLink,textDecoration:'none'}}>{t('superLink.support')}</a></p>
