  ...
  return (
    <div style={{ background: 'var(--sap-cobalt-deep)', minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
      <nav style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 40px', borderBottom: '1px solid rgba(255,255,255,.06)' }}>
        <a href="/" style={{ fontFamily: "'Sora',sans-serif", fontSize: 20, fontWeight: 900, color: '#fff', textDecoration: 'none' }}>SuperAd<span style={{ color: 'var(--sap-accent-light)' }}>{t('common.pro')}</span></a>
      ...
      <div style={{ textAlign: 'center', padding: '0 20px 32px' }}>
        <a href="/register" style={{ display: 'inline-block', padding: '14px 36px', borderRadius: 12, background: 'linear-gradient(135deg,#0ea5e9,#38bdf8)', color: '#fff', fontWeight: 800, fontSize: 16, textDecoration: 'none', boxShadow: '0 0 40px rgba(14,165,233,.25)' }}>{t('incomeGrid3D.joinFree')}</a>
      </div>
    </div>
  );
}
