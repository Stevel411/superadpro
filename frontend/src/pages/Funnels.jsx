  ...
  return (
    <AppLayout>
      ...
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:16,marginBottom:28}}>
        <div style={{background:'linear-gradient(135deg,#0c1222,#1e3a5f)',borderRadius:16,padding:28,position:'relative',overflow:'hidden',border:'1px solid rgba(14,165,233,.15)'}}>
          ...
          <Sparkles size={28} color="#0ea5e9" style={{marginBottom:12}}/>
          <h3 style={{fontFamily:'Sora,sans-serif',fontSize:18,fontWeight:800,color:'#fff',margin:'0 0 6px'}}>AI Funnel Generator</h3>
          <p style={{fontSize:13,color:'#94a3b8',lineHeight:1.6,marginBottom:18}}>Answer 4 questions and AI generates a complete landing page with email capture</p>
          <button onClick={()=>navigate('/pro/ai-funnel')} style={{padding:'11px 24px',borderRadius:10,border:'none',cursor:'pointer',background:'linear-gradient(135deg,#0ea5e9,#38bdf8)',color:'#fff',fontFamily:'Sora,sans-serif',fontSize:13,fontWeight:700,boxShadow:'0 4px 14px rgba(14,165,233,.3)'}}>Create AI Funnel →</button>
        </div>
        <div style={{background:'#f8f9fb',borderRadius:16,padding:28,position:'relative',overflow:'hidden',border:'1px solid #e8ecf2'}}>
          <div style={{position:'absolute',top:10,right:10,width:80,height:80,border:'1px dashed #e2e8f0',borderRadius:12,opacity:.5}}/>
          <LayoutGrid size={28} color="#94a3b8" style={{marginBottom:12}}/>
          <h3 style={{fontFamily:'Sora,sans-serif',fontSize:18,fontWeight:800,color:'#0f172a',margin:'0 0 6px'}}>Blank Canvas</h3>
          <p style={{fontSize:13,color:'#94a3b8',lineHeight:1.6,marginBottom:18}}>Start from scratch with drag-and-drop — full creative freedom</p>
          <button onClick={()=>createFromTemplate('blank')} disabled={creating} style={{padding:'11px 24px',borderRadius:10,border:'1px solid #e2e8f0',cursor:'pointer',background:'#fff',color:'#0f172a',fontFamily:'Sora,sans-serif',fontSize:13,fontWeight:700}}><Plus size={14} style={{marginRight:4}}/> Open SuperPages</button>
        </div>
      </div>
      ...
      {pages.length>0&&(
        <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:12,marginBottom:28}}>
          {[{label:'ACTIVE PAGES',val:published,sub:'published',color:'#16a34a',icon:Zap},{label:'TOTAL LEADS',val:totalLeads,sub:'emails captured',color:'#0ea5e9',icon:Target},{label:'TOTAL VIEWS',val:totalViews,sub:'page visits',color:'#8b5cf6',icon:TrendingUp}].map((s,i)=>(
            <div key={i} style={{background:'#fff',border:'1px solid #e8ecf2',borderRadius:12,padding:'16px 20px',borderLeft:`3px solid ${s.color}`}}>
              <div style={{display:'flex',alignItems:'center',justifyContent:'space-between'}}>
                <div style={{fontSize:10,fontWeight:700,letterSpacing:1,color:'#94a3b8',textTransform:'uppercase'}}>{s.label}</div>
                <s.icon size={16} color={s.color}/>
              </div>
              <div style={{fontFamily:'Sora,sans-serif',fontSize:28,fontWeight:900,color:'#0f172a',margin:'4px 0 2px'}}>{s.val}</div>
      ...
      {pages.length>0&&(
        <>
          <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:14}}>
            <h2 style={{margin:0,fontFamily:'Sora,sans-serif',fontSize:16,fontWeight:800,color:'#0f172a'}}>Your Pages</h2>
            <span style={{fontSize:12,color:'#94a3b8'}}>{pages.length} page{pages.length!==1?'s':''}</span>
          </div>
          <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(300px,1fr))',gap:14,marginBottom:32}}>
            {pages.map(p=>(
              <div key={p.id} style={{background:'#fff',border:'1px solid #e8ecf2',borderRadius:10,overflow:'hidden',boxShadow:'0 2px 8px rgba(0,0,0,.03)',display:'flex',flexDirection:'column'}}>
                <div style={{padding:'14px 16px',borderBottom:'1px solid #f1f3f7'}}>
                  <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:4}}>
                    <div style={{fontSize:14,fontWeight:800,color:'#0f172a',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap',flex:1}}>{p.title||'Untitled'}</div>
                    <span style={{fontSize:9,fontWeight:700,padding:'3px 8px',borderRadius:4,marginLeft:8,flexShrink:0,...(p.status==='published'?{background:'rgba(22,163,74,.08)',color:'#16a34a'}:{background:'#f8f9fb',color:'#94a3b8'})}}>{p.status==='published'?'● Live':'○ Draft'}</span>
                  </div>
                ...
                <div style={{padding:'10px 16px',display:'flex',gap:16,borderBottom:'1px solid #f1f3f7'}}>
                  <div style={{display:'flex',alignItems:'center',gap:5}}><Eye size={13} color="#94a3b8"/><span style={{fontSize:13,fontWeight:700,color:'#0f172a'}}>{p.views||0}</span><span style={{fontSize:11,color:'#94a3b8'}}>Views</span></div>
                  <div style={{display:'flex',alignItems:'center',gap:5}}><FileText size={13} color="#94a3b8"/><span style={{fontSize:13,fontWeight:700,color:'#0f172a'}}>{p.leads_captured||0}</span><span style={{fontSize:11,color:'#94a3b8'}}>Leads</span></div>
                ...
                <div style={{padding:'10px 16px',display:'flex',gap:6,marginTop:'auto'}}>
                  <a href={`/pro/funnel/${p.id}/edit`} style={{flex:1,display:'flex',alignItems:'center',justifyContent:'center',gap:5,padding:'8px 12px',borderRadius:6,fontSize:12,fontWeight:700,textDecoration:'none',background:'#0ea5e9',color:'#fff'}}><Pencil size={12}/> Edit</a>
                  {p.status==='published'&&p.slug&&(<a href={`/p/${p.slug}`} target="_blank" rel="noopener noreferrer" style={{display:'flex',alignItems:'center',justifyContent:'center',gap:5,padding:'8px 12px',borderRadius:6,fontSize:12,fontWeight:700,textDecoration:'none',background:'#f8f9fb',color:'#0f172a',border:'1px solid #e8ecf2'}}><ExternalLink size={12}/> View</a>)}
                  <button onClick={()=>duplicatePage(p.id)} title="Duplicate" style={{display:'flex',alignItems:'center',justifyContent:'center',padding:'8px 10px',borderRadius:6,border:'1px solid #e8ecf2',background:'#f8f9fb',cursor:'pointer'}}><Copy size={13} color="#64748b"/></button>
                  <button onClick={()=>deletePage(p.id,p.title)} title="Delete" style={{display:'flex',alignItems:'center',justifyContent:'center',padding:'8px 10px',borderRadius:6,border:'1px solid #fecaca',background:'#fef2f2',cursor:'pointer'}}><Trash2 size={13} color="#dc2626"/></button>
                </div>
              </div>
            ))}
          </div>
      ...
      <div style={{marginBottom:12,display:'flex',alignItems:'center',gap:8}}>
        <h2 style={{margin:0,fontFamily:'Sora,sans-serif',fontSize:16,fontWeight:800,color:'#0f172a'}}>Start from a Template</h2>
        <span style={{fontSize:11,color:'#94a3b8'}}>Professional layouts ready to customise in SuperPages</span>
      </div>
          ...
          return (
            <div key={t.key} className="tpl-card" onClick={()=>!creating&&createFromTemplate(t.key)} style={{background:'#fff',borderRadius:12,overflow:'hidden',border:'1px solid #e8ecf2',cursor:'pointer',boxShadow:'0 2px 8px rgba(0,0,0,.03)',transition:'all .2s',display:'flex',flexDirection:'column'}}>
              <div style={{height:160,position:'relative'}}>
                ...
                <div style={{position:'absolute',inset:0,background:'linear-gradient(180deg,transparent 50%,rgba(0,0,0,.4) 100%)',borderRadius:'10px 10px 0 0',display:'flex',alignItems:'flex-end',padding:12}}>
                  <div style={{width:28,height:28,borderRadius:7,background:`${t.color}22`,border:`1px solid ${t.color}44`,display:'flex',alignItems:'center',justifyContent:'center'}}><Icon size={14} color={t.color}/></div>
                </div>
              </div>
              <div style={{padding:'14px 16px',flex:1,display:'flex',flexDirection:'column'}}>
                <div style={{fontSize:14,fontWeight:800,color:'#0f172a',marginBottom:4}}>{t.title}</div>
                <div style={{fontSize:12,color:'#94a3b8',lineHeight:1.5,flex:1}}>{t.desc}</div>
                <div style={{marginTop:12}}>
                  <div style={{padding:'8px 16px',borderRadius:8,fontSize:11,fontWeight:700,background:isCreating?'#f1f5f9':`${t.color}10`,color:isCreating?'#94a3b8':t.color,border:`1px solid ${isCreating?'#e2e8f0':t.color+'25'}`,textAlign:'center'}}>{isCreating?'Creating...':'Use This Template →'}</div>
                </div>
              </div>
            </div>
          );
