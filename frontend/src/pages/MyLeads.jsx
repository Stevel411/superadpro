...
function SeqTab({sequences,refresh,flash}) {
  ...
  function genAI(){var n=niche.trim();if(!n){setNicheOpen(true);return;}setGen(true);setNicheOpen(false);apiPost('/api/leads/ai-sequence',{niche:n,tone:'friendly',count:5}).then(function(r){var emails=(r&&r.emails)||[];if(!emails.length){throw new Error('No emails returned');}setEd('new');setTitle(n.charAt(0).toUpperCase()+n.slice(1)+' Welcome Series');setEm(emails);setGen(false);setNiche('');flash('AI wrote your sequence — review, edit, then save');}).catch(function(e){flash(e.message||'AI writer unavailable — try again','err');setGen(false);});}
