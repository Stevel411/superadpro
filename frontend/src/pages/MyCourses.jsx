  ...
  return (
    <AppLayout title={t("courses.myCourses")} subtitle={t("courses.title")}>
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:14,marginBottom:24}}>
        {[
          {value:owned.length,label:t('courses.coursesOwned'),color:'#8b5cf6',bg:'#f5f3ff',border:'#e9d5ff'},
          {value:st.course_sale_count||0,label:t('courses.courseSales'),color:'#16a34a',bg:'#f0fdf4',border:'#dcfce7'},
          {value:'$'+(st.total_earned||0).toFixed(0),label:t('courses.courseEarnings'),color:'#0ea5e9',bg:'#f0f9ff',border:'#bae6fd'},
          {value:(st.direct_commissions||0)+(st.passup_commissions||0),label:t('courses.totalCommissions'),color:'#f59e0b',bg:'#fffbeb',border:'#fef3c7'},
        ].map(function(s, i) {
          return (
