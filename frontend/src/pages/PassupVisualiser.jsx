...
export default function PassupVisualiser() {
  ...
  useEffect(() => { apiGet(EPS['PassupVisualiser']).then(r => { setD(r); setLoading(false); }).catch(() => setLoading(false)); }, []);
  if (loading) return <AppLayout title={TITLES['PassupVisualiser']}><div style={{display:'flex',justifyContent:'center',padding:80}}><div style={{width:40,height:40,border:'3px solid #e5e7eb',borderTopColor:'#0ea5e9',borderRadius:'50%',animation:'spin .8s linear infinite'}}/><style>{`@keyframes spin{to{transform:rotate(360deg)}}`}</style></div></AppLayout>;
  return (
    <AppLayout title={TITLES['PassupVisualiser']} subtitle={SUBS['PassupVisualiser']}>
