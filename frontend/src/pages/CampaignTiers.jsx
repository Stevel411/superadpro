...
export default function CampaignTiers() {
  ...
  return (
    <AppLayout title="Campaign Tiers" subtitle="Activate tiers to advertise your videos and earn grid commissions">
      <style>{`
        @keyframes ctPop{0%{opacity:0;transform:scale(.92) translateY(12px)}100%{opacity:1;transform:scale(1) translateY(0)}}
        .ct-card{animation:ctPop .7s ease both}
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:12,marginBottom:12}}>
        {tiers.slice(0,4).map(function(t, i) {
          return <TierCard key={t.tier} tier={t} colors={TIER_COLORS[i]} isLast={false} delay={i * 0.1}/>;
        })}
      </div>
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:12,marginBottom:20}}>
        {tiers.slice(4).map(function(t, i) {
          return <TierCard key={t.tier} tier={t} colors={TIER_COLORS[i + 4]} isLast={i === 3} delay={(i + 4) * 0.1}/>;
        })}
      </div>
