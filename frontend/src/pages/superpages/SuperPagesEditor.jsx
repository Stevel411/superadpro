    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center' }}
      ...
      <div onClick={e => e.stopPropagation()}
        ...
        <div style={{ padding: 20 }}>
          {/* Text/Heading — textarea for HTML */}
          {['heading', 'text', 'label', 'review', 'testimonial', 'faq', 'stat', 'icontext', 'separator', 'logostrip'].includes(type) && (
            <>
              <label style={{ display: 'block', fontSize: 12, fontWeight: 700, color: '#475569', marginBottom: 4 }}>Content (HTML)</label>
              <textarea value={localTxt} onChange={e => setLocalTxt(e.target.value)}
                rows={8}
                style={{ width: '100%', padding: 12, border: '2px solid #e2e8f0', borderRadius: 10, fontSize: 13, fontFamily: 'monospace', outline: 'none', resize: 'vertical', boxSizing: 'border-box', marginBottom: 12 }} />
            </>
          )}
