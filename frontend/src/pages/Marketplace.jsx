    ...
    <div>
        ...
        <div style={{position:'relative',zIndex:1,display:'flex',alignItems:'center',justifyContent:'space-between',flexWrap:'wrap',gap:20}}>
          <div>
            <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:14}}>
              <div style={{width:32,height:32,borderRadius:8,background:'rgba(14,165,233,.15)',display:'flex',alignItems:'center',justifyContent:'center'}}><Store size={18} color="#38bdf8"/></div>
          ...
          <div style={{display:'flex',gap:10,alignItems:'center'}}>
            <StatBox value={allProducts.length} label="Products" color="#38bdf8"/>
            <StatBox value="25%" label="Commission" color="#0ea5e9"/>
            <div style={{display:'flex',flexDirection:'column',gap:6}}>
              <button onClick={onMyProducts} style={{padding:'10px 18px',borderRadius:8,border:'1px solid rgba(14,165,233,.3)',background:'rgba(14,165,233,.08)',color:'#38bdf8',fontSize:11,fontWeight:800,cursor:'pointer',fontFamily:'inherit'}}>My Products</button>
      ...
      <div style={{marginTop:32,background:'#fff',border:'1px solid #e8ecf2',borderRadius:14,overflow:'hidden'}}>
        ...
        <div style={{padding:'24px',display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:16}}>
          {[
            {icon:Package,label:'Upload',desc:'Upload your digital product file',color:'#8b5cf6'},
            {icon:Eye,label:'List',desc:'Set price, write sales page, submit for review',color:'#0ea5e9'},
            {icon:Users,label:'Promote',desc:'Affiliates share your product link',color:'#f59e0b'},
            {icon:DollarSign,label:'Earn',desc:'50% to you, 25% to affiliate, 25% platform',color:'#16a34a'},
          ].map(function(s,i) {
            var Icon=s.icon;
            return (
          ...
          {Array.isArray(p.features) && p.features.length > 0 && (
            <div style={{background:'#fff',border:'1px solid #e8ecf2',borderRadius:14,overflow:'hidden',marginTop:14}}>
              ...
              <div style={{padding:'16px 20px'}}>
                {p.features.map(function(f,i){return <div key={i} style={{display:'flex',gap:8,padding:'6px 0',fontSize:13,color:'#334155'}}><span style={{color:'#16a34a'}}>✓</span>{f}</div>;})}
              </div>
            </div>
          )}
        </div>
          ...
          <div style={{background:'#fff',border:'1px solid #e8ecf2',borderRadius:14,overflow:'hidden'}}>
            ...
            <div style={{padding:'20px'}}>
              ...
              <div style={{display:'flex',gap:6,marginBottom:10}}>
                ...
                <button onClick={copyLink} style={{display:'flex',alignItems:'center',gap:3,padding:'8px 14px',borderRadius:8,border:'none',cursor:'pointer',background:copied?'#16a34a':'#0ea5e9',color:'#fff',fontSize:11,fontWeight:700,fontFamily:'inherit'}}>
