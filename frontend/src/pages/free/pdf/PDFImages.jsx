      ...
      <div style={{ maxWidth: 600, width: '100%' }}>
        ...
        {images.length > 0 && (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 8, marginBottom: 16 }}>
            {images.map((img, i) => (
              <div key={img.id} style={{ position: 'relative', borderRadius: 8, overflow: 'hidden', border: '1px solid #2a3040', background: '#1b2030' }}>
                <img src={img.src} alt="" style={{ width: '100%', aspectRatio: '1', objectFit: 'cover', display: 'block' }} />
                <div style={{ position: 'absolute', top: 4, left: 4, fontSize: 9, fontWeight: 700, background: '#0ea5e9', color: '#fff', padding: '1px 5px', borderRadius: 4 }}>{i + 1}</div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10, marginBottom: 16 }}>
          <div>
            ...
            <div style={{ display: 'flex', gap: 4 }}>
              {['A4', 'Letter'].map(s => (
                <button key={s} onClick={() => setPageSize(s)} style={{ flex: 1, padding: '7px 0', borderRadius: 7, border: 'none', cursor: 'pointer', fontFamily: '"DM Sans",sans-serif', fontSize: 11, fontWeight: pageSize === s ? 700 : 600, background: pageSize === s ? '#0ea5e9' : '#1b2030', color: pageSize === s ? '#fff' : '#7b8594', borderWidth: 1, borderStyle: 'solid', borderColor: pageSize === s ? '#0ea5e9' : '#2a3040' }}>{s}</button>
              ))}
            </div>
          </div>
          <div>
            ...
            <div style={{ display: 'flex', gap: 4 }}>
              {['portrait', 'landscape'].map(o => (
                <button key={o} onClick={() => setOrientation(o)} style={{ flex: 1, padding: '7px 0', borderRadius: 7, border: 'none', cursor: 'pointer', fontFamily: '"DM Sans",sans-serif', fontSize: 10, fontWeight: orientation === o ? 700 : 600, background: orientation === o ? '#0ea5e9' : '#1b2030', color: orientation === o ? '#fff' : '#7b8594', borderWidth: 1, borderStyle: 'solid', borderColor: orientation === o ? '#0ea5e9' : '#2a3040', textTransform: 'capitalize' }}>{o}</button>
              ))}
            </div>
          </div>
          <div>
        ...
        <button onClick={buildPDF} disabled={images.length === 0 || building}
          style={{
            ...
            background: images.length === 0 ? '#1b2030' : '#0ea5e9', color: images.length === 0 ? '#7b8594' : '#fff',
