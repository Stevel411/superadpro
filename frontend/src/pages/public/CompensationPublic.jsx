...
// Public dynamic compensation plan — /compensation (23 Jun 2026).
// Light "Daylight" theme. Interactive: tier selector recomputes commissions
// live; referral slider computes membership income. All rates are the verified
// live v2 constants (50% direct / 5% x5 uni-level / 25% bonus pool = 4x tier).
// English-first; i18n pass to follow. CSS scoped under .comp-plan (cp- prefix).

var TIERS = [
  { n: 'Starter', p: 20 }, { n: 'Builder', p: 50 }, { n: 'Pro', p: 100 }, { n: 'Advanced', p: 200 },
  { n: 'Premium', p: 400 }, { n: 'Elite', p: 600 }, { n: 'Master', p: 800 }, { n: 'Champion', p: 1000 }
];
var BONUS_SEATS = [4, 8, 12, 16];
function money(x) { return '$' + x.toLocaleString('en-US', { minimumFractionDigits: (x % 1 ? 2 : 0), maximumFractionDigits: 2 }); }

export default function CompensationPublic() {
  var _t = useState(TIERS[0]); var tier = _t[0]; var setTier = _t[1];
  var _r = useState(2); var refs = _r[0]; var setRefs = _r[1];

  var p = tier.p;
  var refMonthly = refs * 10;
  var refHint;
  if (refs === 0) refHint = 'Refer members to start earning $10/mo each.';
  else if (refMonthly < 20) refHint = '1 more referral covers your $20/mo membership.';
  else if (refMonthly === 20) refHint = 'Just 2 referrals cover your own $20/mo membership.';
  else refHint = money(refMonthly) + '/mo recurring — ' + money(refMonthly - 20) + '/mo after your membership.';

  return (
    <>
      <style>{CSS_COMP}</style>
      <div className="comp-plan">

        {/* NAV */}
        <div className="cp-nav"><div className="cp-wrap cp-nav-in">
          <Link to="/" className="cp-brand">
            <span className="cp-mk"><svg viewBox="0 0 24 24" fill="#fff"><path d="M8 5v14l11-7z" /></svg></span>
            <span className="cp-nm">SuperAd<em>Pro</em></span>
          </Link>
          <div className="cp-right">
            <Link to="/explore" className="cp-navlink">Explore</Link>
            <Link to="/register" className="cp-btn cp-pri cp-nav-cta">Create account</Link>
          </div>
        </div></div>

        {/* HERO */}
        <section className="cp-hero"><div className="cp-wrap">
          <span className="cp-eyebrow">The compensation plan</span>
          <h1>See exactly how<br /><span className="cp-g">the money is shared.</span></h1>
          <p className="cp-sub">SuperAdPro is an advertising platform you can earn from. Pick a tier below and see the real numbers — 100% of Campaign Grid revenue is shared with members, with nothing hidden.</p>
          <span className="cp-disc-pill">⚠️ Earnings depend on individual effort — no income is guaranteed.</span>
        </div></section>

        {/* ADVERTISING FRAMING */}
        <section className="cp-sec"><div className="cp-wrap">
          <div className="cp-head"><span className="cp-eyebrow">First, what you're buying</span><h2>It's an advertising platform first</h2><p>The Campaign Grid isn't a money game — it's how SuperAdPro shares its advertising revenue with the members who power it.</p></div>
          <div className="cp-adrow">
            <div className="cp-adcard"><div className="cp-an">1</div><h4>Buy a campaign tier</h4><p>Activate a tier to put your content into the advertising rotation in front of real members.</p></div>
            <div className="cp-adcard"><div className="cp-an">2</div><h4>Real members watch</h4><p>Every member watches daily to stay qualified — a genuinely engaged audience, not bots.</p></div>
            <div className="cp-adcard"><div className="cp-an">3</div><h4>100% is shared back</h4><p>50% direct · 25% across five levels · 25% bonus pool. The company keeps 0%.</p></div>
          </div>
        </div></section>

        {/* CALCULATOR */}
        <section className="cp-sec cp-alt"><div className="cp-wrap">
          <div className="cp-head"><span className="cp-eyebrow">Interactive</span><h2>Explore the Campaign Grid</h2><p>Tap a tier to see how each commission is calculated. Every figure is the live platform rate.</p></div>
          <div className="cp-calc">
            <div className="cp-calc-top">
              <div><h3>Campaign Grid calculator</h3><p>Tier: <span className="cp-mono">{tier.n}</span> · seat price <span className="cp-mono">${p}</span></p></div>
              <div className="cp-badge100">100% to members · 0% company</div>
            </div>
            <div className="cp-tiers">
              {TIERS.map(function (t) {
                return (
                  <button key={t.n} type="button" className={'cp-tierbtn' + (t.n === tier.n ? ' on' : '')} onClick={function () { setTier(t); }}>
                    {t.n}<small>${t.p}</small>
                  </button>
                );
              })}
            ...
            <div className="cp-calc-body">
              <div className="cp-outs">
                <div className="cp-out d"><div className="cp-oic">50%</div><div><div className="cp-ot">Direct commission — per member you personally sponsor</div><div className="cp-ov">{money(p * 0.5)}</div></div></div>
                <div className="cp-out u"><div className="cp-oic">5%</div><div><div className="cp-ot">Uni-level — per qualifying member, on each of 5 levels</div><div className="cp-ov">{money(p * 0.05)} <small>× 5 levels = {money(p * 0.25)}</small></div></div></div>
                <div className="cp-out b"><div className="cp-oic">25%</div><div><div className="cp-ot">Bonus pool — paid across a completed 16-seat grid</div><div className="cp-ov">{money(p * 4)}</div></div></div>
              </div>
              <div className="cp-gridbox">
                <div className="cp-gl">4 × 4 grid · bonus paid at seats 4 · 8 · 12 · 16</div>
                <div className="cp-g4">
                  {Array.from({ length: 16 }, function (_, i) { return i + 1; }).map(function (s) {
                    return <div key={s} className={'cp-cell' + (BONUS_SEATS.indexOf(s) >= 0 ? ' bonus' : '')}>{s}</div>;
                  })}
                </div>
                <div className="cp-gcap">Completed grid bonus pool: <b>{money(p * 4)}</b></div>
              </div>
            </div>
          </div>
        </div></section>
        ...
        {/* THREE STREAMS */}
        <section className="cp-sec"><div className="cp-wrap">
          <div className="cp-head"><span className="cp-eyebrow">All together</span><h2>Three ways the money comes back</h2></div>
          <div className="cp-streams">
            <div className="cp-scard s1"><div className="cp-strip"></div><div className="cp-tag">Stream 1 · Membership</div><h4>Refer, earn monthly</h4><p>Earn $10/month for every active member you refer — recurring, for as long as they stay.</p>
              <div className="cp-slider-wrap">
                <div className="cp-slider-row"><span className="cp-slider-lab">{refs} active {refs === 1 ? 'referral' : 'referrals'}</span><span className="cp-rv">${refMonthly}<small>/mo</small></span></div>
                <input type="range" min="0" max="20" value={refs} onChange={function (e) { setRefs(+e.target.value); }} />
                <div className="cp-hint">{refHint}</div>
              </div>
            </div>
            <div className="cp-scard s2"><div className="cp-strip"></div><div className="cp-tag">Stream 2 · Campaign Grid</div><h4>100% to members</h4><p>A revenue-share advertising product. Across 8 tiers, the completed-grid bonus pool ranges from $80 to $4,000 — all shared with members.</p></div>
            <div className="cp-scard s3"><div className="cp-strip"></div><div className="cp-tag">Stream 3 · Creator Credits</div><h4>20% flat share</h4><p>Earn 20% on every Creator Credit pack your referrals buy — their first purchase and every repurchase after.</p></div>
          </div>
        </div></section>

        {/* DISCLAIMER */}
        <section className="cp-sec cp-alt" style={{ padding: '60px 0' }}><div className="cp-wrap">
          <div className="cp-disc-box"><b>Income disclosure.</b> The figures above are commission rates, not earnings projections. Actual income depends entirely on individual effort, the size and activity of your network, and grid completion. Many members earn little or nothing. "100% to members" applies to the Campaign Grid advertising product. No income is guaranteed.</div>
        </div></section>

        {/* FINAL */}
        <section className="cp-final"><div className="cp-wrap">
          <h2>Ready to start?</h2>
          <Link to="/register" className="cp-btn cp-pri cp-final-btn">Create your free account &nbsp;→</Link>
        </div></section>
        <div className="cp-foot">© 2026 SuperAdPro · Earnings depend on individual effort; no income is guaranteed.</div>
