...
export default function Account() {
  ...
  var iS = {width:'100%',padding:'10px 14px',border:'2px solid #e8ecf2',borderRadius:10,fontSize:13,color:'var(--sap-text-primary)',fontFamily:'inherit',background:'#fff',boxSizing:'border-box',outline:'none',transition:'border .2s'};
  function foc(e){e.target.style.borderColor='var(--sap-accent)';} function blu(e){e.target.style.borderColor='var(--sap-border-light)';}
  ...
  return (
    <AppLayout title={t("account.title")} subtitle={t("account.subtitle")}>
      {toast && <div style={{borderRadius:10,padding:'10px 16px',marginBottom:14,fontSize:13,fontWeight:700,...(toast.type==='ok'?{background:'var(--sap-green-bg-mid)',color:'var(--sap-green)'}:{background:'var(--sap-red-bg)',color:'var(--sap-red)'})}}>{toast.msg}</div>}
      ...
      <div className="grid-3-col" style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:14,alignItems:'stretch'}}>
        ...
        <Card title={t("account.profile")}>
          <div style={{display:'flex',alignItems:'center',gap:14,marginBottom:14,paddingBottom:14,borderBottom:'1px solid #f1f3f7'}}>
            <div style={{position:'relative',flexShrink:0}}>
              ...
              <div style={{position:'absolute',bottom:-2,right:-2,width:20,height:20,borderRadius:'50%',background:'var(--sap-accent)',border:'2px solid #fff',display:'flex',alignItems:'center',justifyContent:'center'}}><span style={{fontSize:13,color:'#fff',fontWeight:800}}>+</span></div>
              <input type="file" accept="image/*" onChange={handleAvatarUpload} style={{display:'none'}}/>
              </label>
              {avatarUrl && <button onClick={removeAvatar} title={t('account.removePhoto')} style={{position:'absolute',top:-4,right:-4,width:18,height:18,borderRadius:'50%',background:'var(--sap-red-bright)',border:'2px solid #fff',display:'flex',alignItems:'center',justifyContent:'center',cursor:'pointer',fontSize:13,color:'#fff',fontWeight:800,lineHeight:1,padding:0,zIndex:2}}>✕</button>}
            </div>
            <div>
              <div style={{fontSize:16,fontWeight:800,color:'var(--sap-text-primary)'}}>{user.first_name||''} {user.last_name||''}</div>
              <div style={{fontSize:13,color:'var(--sap-text-muted)'}}>@{user.username}</div>
              <span style={{display:'inline-block',fontSize:13,fontWeight:700,padding:'2px 8px',borderRadius:4,marginTop:3,...(user.is_active?{background:'var(--sap-green-bg-mid)',color:'var(--sap-green)'}:{background:'var(--sap-amber-bg)',color:'var(--sap-amber-dark)'})}}>{user.is_active?t('account.activeStatus'):t('account.inactiveStatus')}</span>
            </div>
          </div>
        ...
        <Card title={t("account.editProfile")}>
          <div style={{marginBottom:12}}><label style={{fontSize:13,fontWeight:700,color:'var(--sap-text-muted)',textTransform:'uppercase',letterSpacing:.5,display:'block',marginBottom:4}}>{t("account.firstName")}</label><input value={firstName} onChange={function(e){setFirstName(e.target.value);}} style={iS} onFocus={foc} onBlur={blu}/></div>
          <div style={{marginBottom:12}}><label style={{fontSize:13,fontWeight:700,color:'var(--sap-text-muted)',textTransform:'uppercase',letterSpacing:.5,display:'block',marginBottom:4}}>{t("account.lastName")}</label><input value={lastName} onChange={function(e){setLastName(e.target.value);}} style={iS} onFocus={foc} onBlur={blu}/></div>
          <div style={{marginBottom:14}}><label style={{fontSize:13,fontWeight:700,color:'var(--sap-text-muted)',textTransform:'uppercase',letterSpacing:.5,display:'block',marginBottom:4}}>{t("account.country")}</label><select value={country} onChange={function(e){setCountry(e.target.value);}} style={iS}><option value="">{t("account.selectCountry")}</option>{COUNTRIES.map(function(c){return <option key={c} value={c}>{c}</option>;})}</select></div>
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12,marginBottom:14}}>
            <div><label style={{fontSize:13,fontWeight:700,color:'var(--sap-text-muted)',textTransform:'uppercase',letterSpacing:.5,display:'block',marginBottom:4}}>{t("account.gender")}</label><select value={gender} onChange={function(e){setGender(e.target.value);}} style={iS}><option value="">{t("account.preferNotToSay")}</option><option value="male">{t("account.male")}</option><option value="female">{t("account.female")}</option><option value="other">{t("account.other")}</option></select></div>
          ...
          <div style={{marginBottom:14}}>
            <label style={{fontSize:13,fontWeight:700,color:'var(--sap-text-muted)',textTransform:'uppercase',letterSpacing:.5,display:'block',marginBottom:4}}>{t("account.interests")} <span style={{fontWeight:400,textTransform:'none'}}>{t("account.interestsHelp")}</span></label>
            ...
            <div style={{fontSize:13,color:'var(--sap-text-muted)',marginTop:4}}>{t("account.interestsNote")}</div>
          </div>
          <button onClick={saveProfile} disabled={savingProfile} style={Object.assign({},btnS,{background:'var(--sap-accent)'})}>{savingProfile?t('account.saving'):t('account.saveProfile')}</button>
        </Card>

        <Card title={t("account.changePassword")}>
          <div style={{marginBottom:12}}><label style={{fontSize:13,fontWeight:700,color:'var(--sap-text-muted)',textTransform:'uppercase',letterSpacing:.5,display:'block',marginBottom:4}}>{t("account.currentPassword")}</label><input type="password" value={currentPw} onChange={function(e){setCurrentPw(e.target.value);}} placeholder={t("account.currentPassword")} style={iS} onFocus={foc} onBlur={blu}/></div>
        ...
        <Card title={t("account.kyc")} chip={kyc==='approved'?{t:t('account.verified'),c:'var(--sap-green)'}:kyc==='pending'?{t:t('account.underReview'),c:'var(--sap-amber)'}:kyc==='rejected'?{t:t('account.rejected'),c:'var(--sap-red)'}:{t:t('account.notStarted'),c:'var(--sap-text-muted)'}}>
          {kyc==='approved'?<div style={{textAlign:'center',padding:'20px 0'}}><div style={{fontSize:32,marginBottom:6}}>✅</div><div style={{fontSize:14,fontWeight:700,color:'var(--sap-green)'}}>{t("account.identityVerified")}</div><div style={{fontSize:13,color:'var(--sap-text-muted)'}}>{t("account.approvedWithdrawals")}</div></div>
          :kyc==='pending'?<div style={{textAlign:'center',padding:'20px 0'}}><div style={{fontSize:32,marginBottom:6}}>⏳</div><div style={{fontSize:14,fontWeight:700,color:'var(--sap-amber)'}}>{t("account.underReview").replace("⏳ ","")}</div><div style={{fontSize:13,color:'var(--sap-text-muted)'}}>{t("account.underReviewTime")}</div></div>
          :<>
            {kyc==='rejected'&&<div style={{padding:'8px 12px',background:'var(--sap-red-bg)',border:'1px solid #fecaca',borderRadius:8,marginBottom:10,fontSize:13,color:'var(--sap-red)',fontWeight:600}}>{t("account.rejectedResubmit")}</div>}
            <div>
              <div style={{marginBottom:10}}><label style={{fontSize:13,fontWeight:700,color:'var(--sap-text-muted)',textTransform:'uppercase',letterSpacing:.5,display:'block',marginBottom:4}}>{t("account.dateOfBirth")}</label><input type="date" value={kycDob} onChange={function(e){setKycDob(e.target.value);}} required style={iS} onFocus={foc} onBlur={blu}/></div>
              ...
              <div style={{marginBottom:12}}><label style={{fontSize:13,fontWeight:700,color:'var(--sap-text-muted)',textTransform:'uppercase',letterSpacing:.5,display:'block',marginBottom:4}}>{t("account.uploadId")}</label>
                <label style={{display:'block',width:'100%',padding:10,border:kycFile?'2px solid #0ea5e9':'2px dashed #d1d5db',borderRadius:10,fontSize:13,color:kycFile?'var(--sap-accent)':'var(--sap-text-muted)',background:'#fafbfc',cursor:'pointer',boxSizing:'border-box',textAlign:'center'}}>
              ...
              <button onClick={submitKyc} disabled={savingKyc} style={Object.assign({},btnS,{background:'var(--sap-accent)'})}>{savingKyc?t('account.submitting'):(kyc==='rejected'?t('account.resubmit'):t('account.submitVerification'))}</button>
            </div>
          </>}
        </Card>
        ...
        <Card title={t("account.twoFA")} chip={user.totp_enabled?{t:t('account.twoFAEnabled'),c:'var(--sap-green)'}:{t:t('account.twoFADisabled'),c:'var(--sap-text-muted)'}}>
          {user.totp_enabled?<>
            <div style={{textAlign:'center',padding:'16px 0'}}><div style={{fontSize:32,marginBottom:6}}>🔒</div><div style={{fontSize:14,fontWeight:700,color:'var(--sap-green)'}}>{t("account.twoFAActive")}</div><div style={{fontSize:13,color:'var(--sap-text-muted)'}}>{t("account.accountProtected")}</div></div>
            ...
            <div style={{textAlign:'center',padding:'16px 0'}}><div style={{fontSize:32,marginBottom:6}}>🔓</div><div style={{fontSize:14,fontWeight:700,color:'var(--sap-text-muted)'}}>{t("account.notEnabled")}</div><div style={{fontSize:13,color:'var(--sap-text-muted)'}}>{t("account.requiredWithdrawals")}</div></div>
        ...
        <Card title={t("account.withdrawalWallet")}>
          <div style={{marginBottom:12}}><label style={{fontSize:13,fontWeight:700,color:'var(--sap-text-muted)',textTransform:'uppercase',letterSpacing:.5,display:'block',marginBottom:4}}>{t("account.walletAddressLabel")}</label><input value={walletAddr} onChange={function(e){setWalletAddr(e.target.value);}} placeholder="0x..." style={Object.assign({},iS,{fontFamily:'monospace',fontSize:13})} onFocus={foc} onBlur={blu}/></div>
          <div style={{padding:'10px 12px',background:'var(--sap-bg-input)',border:'1px solid #e8ecf2',borderRadius:8,marginBottom:12,fontSize:13,color:'var(--sap-text-muted)',lineHeight:1.6}}>{t("account.walletSupported")}</div>
...
function Card({title, chip, children}) {
  return (
    <div style={{background:'#fff',border:'1px solid #e8ecf2',borderRadius:12,overflow:'hidden',display:'flex',flexDirection:'column',boxShadow:'0 2px 8px rgba(0,0,0,.03)'}}>
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'12px 16px',background:'var(--sap-cobalt-deep)'}}>
        <div style={{fontSize:13,fontWeight:800,color:'#fff'}}>{title}</div>
        {chip&&<span style={{fontSize:13,fontWeight:700,padding:'2px 8px',borderRadius:4,background:chip.c+'20',color:chip.c}}>{chip.t}</span>}
      </div>
...
function Row({k, v, mono, last}) {
  return (
    <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'6px 0',borderBottom:last?'none':'1px solid #f5f6f8'}}>
      <span style={{fontSize:13,fontWeight:700,color:'var(--sap-text-muted)'}}>{k}</span>
      <span style={{fontSize:12,fontWeight:600,color:mono?'var(--sap-accent)':'var(--sap-text-primary)',...(mono?{fontFamily:'monospace'}:{})}}>{v}</span>
    </div>
  );
}
