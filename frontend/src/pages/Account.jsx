    ...
    <AppLayout title={t("account.title")} subtitle={t("account.subtitle")}>
      ...
      <div className="grid-3-col" style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:14,alignItems:'stretch'}}>
        ...
        <Card title={t("account.profile")}>
          <div style={{display:'flex',alignItems:'center',gap:14,marginBottom:14,paddingBottom:14,borderBottom:'1px solid #f1f3f7'}}>
            ...
            <div>
              ...
              <div style={{fontSize:11,color:'var(--sap-text-muted)'}}>@{user.username}</div>
              <span style={{display:'inline-block',fontSize:9,fontWeight:700,padding:'2px 8px',borderRadius:4,marginTop:3,...(user.is_active?{background:'var(--sap-green-bg-mid)',color:'var(--sap-green)'}:{background:'var(--sap-amber-bg)',color:'var(--sap-amber-dark)'})}}>{user.is_active?t('account.activeStatus'):t('account.inactiveStatus')}</span>
            </div>
          </div>
        ...
        <Card title={t("account.editProfile")}>
          <div style={{marginBottom:12}}><label style={{fontSize:10,fontWeight:700,color:'var(--sap-text-muted)',textTransform:'uppercase',letterSpacing:.5,display:'block',marginBottom:4}}>{t("account.firstName")}</label><input value={firstName} onChange={function(e){setFirstName(e.target.value);}} style={iS} onFocus={foc} onBlur={blu}/></div>
          <div style={{marginBottom:12}}><label style={{fontSize:10,fontWeight:700,color:'var(--sap-text-muted)',textTransform:'uppercase',letterSpacing:.5,display:'block',marginBottom:4}}>{t("account.lastName")}</label><input value={lastName} onChange={function(e){setLastName(e.target.value);}} style={iS} onFocus={foc} onBlur={blu}/></div>
          <div style={{marginBottom:14}}><label style={{fontSize:10,fontWeight:700,color:'var(--sap-text-muted)',textTransform:'uppercase',letterSpacing:.5,display:'block',marginBottom:4}}>{t("account.country")}</label><select value={country} onChange={function(e){setCountry(e.target.value);}} style={iS}><option value="">{t("account.selectCountry")}</option>{COUNTRIES.map(function(c){return <option key={c} value={c}>{c}</option>;})}</select></div>
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12,marginBottom:14}}>
            <div><label style={{fontSize:10,fontWeight:700,color:'var(--sap-text-muted)',textTransform:'uppercase',letterSpacing:.5,display:'block',marginBottom:4}}>{t("account.gender")}</label><select value={gender} onChange={function(e){setGender(e.target.value);}} style={iS}><option value="">{t("account.preferNotToSay")}</option><option value="male">{t("account.male")}</option><option value="female">{t("account.female")}</option><option value="other">{t("account.other")}</option></select></div>
          ...
          <div style={{marginBottom:14}}>
            <label style={{fontSize:10,fontWeight:700,color:'var(--sap-text-muted)',textTransform:'uppercase',letterSpacing:.5,display:'block',marginBottom:4}}>{t("account.interests")} <span style={{fontWeight:400,textTransform:'none'}}>{t("account.interestsHelp")}</span></label>
            ...
            <div style={{fontSize:10,color:'var(--sap-text-muted)',marginTop:4}}>{t("account.interestsNote")}</div>
          </div>
        ...
        <Card title={t("account.kyc")} chip={kyc==='approved'?{t:t('account.verified'),c:'var(--sap-green)'}:kyc==='pending'?{t:t('account.underReview'),c:'var(--sap-amber)'}:kyc==='rejected'?{t:t('account.rejected'),c:'var(--sap-red)'}:{t:t('account.notStarted'),c:'var(--sap-text-muted)'}}>
          {kyc==='approved'?<div style={{textAlign:'center',padding:'20px 0'}}><div style={{fontSize:32,marginBottom:6}}>✅</div><div style={{fontSize:14,fontWeight:700,color:'var(--sap-green)'}}>{t("account.identityVerified")}</div><div style={{fontSize:11,color:'var(--sap-text-muted)'}}>{t("account.approvedWithdrawals")}</div></div>
          :kyc==='pending'?<div style={{textAlign:'center',padding:'20px 0'}}><div style={{fontSize:32,marginBottom:6}}>⏳</div><div style={{fontSize:14,fontWeight:700,color:'var(--sap-amber)'}}>{t("account.underReview").replace("⏳ ","")}</div><div style={{fontSize:11,color:'var(--sap-text-muted)'}}>{t("account.underReviewTime")}</div></div>
          :<>
            {kyc==='rejected'&&<div style={{padding:'8px 12px',background:'var(--sap-red-bg)',border:'1px solid #fecaca',borderRadius:8,marginBottom:10,fontSize:11,color:'var(--sap-red)',fontWeight:600}}>{t("account.rejectedResubmit")}</div>}
            <div>
              <div style={{marginBottom:10}}><label style={{fontSize:10,fontWeight:700,color:'var(--sap-text-muted)',textTransform:'uppercase',letterSpacing:.5,display:'block',marginBottom:4}}>{t("account.dateOfBirth")}</label><input type="date" value={kycDob} onChange={function(e){setKycDob(e.target.value);}} required style={iS} onFocus={foc} onBlur={blu}/></div>
              ...
              <div style={{marginBottom:12}}><label style={{fontSize:10,fontWeight:700,color:'var(--sap-text-muted)',textTransform:'uppercase',letterSpacing:.5,display:'block',marginBottom:4}}>{t("account.uploadId")}</label>
                <label style={{display:'block',width:'100%',padding:10,border:kycFile?'2px solid #0ea5e9':'2px dashed #d1d5db',borderRadius:10,fontSize:11,color:kycFile?'var(--sap-accent)':'var(--sap-text-muted)',background:'#fafbfc',cursor:'pointer',boxSizing:'border-box',textAlign:'center'}}>
        ...
        <Card title={t("account.twoFA")} chip={user.totp_enabled?{t:t('account.twoFAEnabled'),c:'var(--sap-green)'}:{t:t('account.twoFADisabled'),c:'var(--sap-text-muted)'}}>
          {user.totp_enabled?<>
            <div style={{textAlign:'center',padding:'16px 0'}}><div style={{fontSize:32,marginBottom:6}}>🔒</div><div style={{fontSize:14,fontWeight:700,color:'var(--sap-green)'}}>{t("account.twoFAActive")}</div><div style={{fontSize:11,color:'var(--sap-text-muted)'}}>{t("account.accountProtected")}</div></div>
            ...
            <div style={{textAlign:'center',padding:'16px 0'}}><div style={{fontSize:32,marginBottom:6}}>🔓</div><div style={{fontSize:14,fontWeight:700,color:'var(--sap-text-muted)'}}>{t("account.notEnabled")}</div><div style={{fontSize:11,color:'var(--sap-text-muted)'}}>{t("account.requiredWithdrawals")}</div></div>
        ...
        <Card title={t("account.withdrawalWallet")}>
          ...
          <div style={{padding:'10px 12px',background:'var(--sap-bg-input)',border:'1px solid #e8ecf2',borderRadius:8,marginBottom:12,fontSize:10,color:'var(--sap-text-muted)',lineHeight:1.6}}>{t("account.walletSupported")}</div>
...
function Row({k, v, mono, last}) {
  return (
    <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'6px 0',borderBottom:last?'none':'1px solid #f5f6f8'}}>
      <span style={{fontSize:11,fontWeight:700,color:'var(--sap-text-muted)'}}>{k}</span>
