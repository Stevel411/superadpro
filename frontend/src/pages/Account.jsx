...
export default function Account() {
  ...
  var iS = {width:'100%',padding:'10px 14px',border:'2px solid #e8ecf2',borderRadius:10,fontSize:15,color:'var(--sap-text-primary)',fontFamily:'inherit',background:'#fff',boxSizing:'border-box',outline:'none',transition:'border .2s'};
  function foc(e){e.target.style.borderColor='var(--sap-accent)';} function blu(e){e.target.style.borderColor='var(--sap-border-light)';}
  var btnS = {padding:'10px 28px',borderRadius:10,fontSize:13,fontWeight:800,border:'none',cursor:'pointer',fontFamily:'inherit',display:'block',margin:'0 auto',color:'#fff'};

  return (
    <AppLayout title={t("account.title")} subtitle={t("account.subtitle")}>
      {toast && <div style={{borderRadius:10,padding:'10px 16px',marginBottom:14,fontSize:15,fontWeight:700,...(toast.type==='ok'?{background:'var(--sap-green-bg-mid)',color:'var(--sap-green)'}:{background:'var(--sap-red-bg)',color:'var(--sap-red)'})}}>{toast.msg}</div>}

      {/* ── Link to Purchases & Holdings (dedicated page, 28 May 2026) ──
          Moved off this page into its own route so Account stays focused on
          settings. The data view lives at /account/purchases. */}
      <Link to="/account/purchases" style={{textDecoration:'none',display:'block',marginBottom:18}}>
        <div style={{background:'var(--sap-bg-card)',border:'1px solid var(--sap-border)',borderRadius:14,padding:'18px 20px',boxShadow:'var(--sap-shadow-sm)',display:'flex',alignItems:'center',gap:16,transition:'border-color .15s,transform .15s'}}
          onMouseEnter={function(e){e.currentTarget.style.borderColor='var(--sap-accent)';e.currentTarget.style.transform='translateY(-1px)';}}
          onMouseLeave={function(e){e.currentTarget.style.borderColor='var(--sap-border)';e.currentTarget.style.transform='none';}}>
          <div style={{width:44,height:44,borderRadius:12,flexShrink:0,display:'flex',alignItems:'center',justifyContent:'center',background:'linear-gradient(135deg,var(--sap-cobalt-deep),var(--sap-cobalt-mid))',color:'#fff',fontSize:20}}>🧾</div>
          <div style={{flex:1}}>
            <div style={{fontFamily:'Sora,sans-serif',fontWeight:800,fontSize:16,color:'var(--sap-text-primary)'}}>{t('account.purchasesTitle',{defaultValue:'Your Purchases & Holdings'})}</div>
            <div style={{fontSize:13,color:'var(--sap-text-faint)',marginTop:2}}>{t('account.purchasesLinkSub',{defaultValue:'See everything you\'ve bought and what\'s active on your account.'})}</div>
          </div>
          <div style={{fontSize:20,color:'var(--sap-accent)',fontWeight:700}}>&rarr;</div>
        </div>
      </Link>
      ...
      <div className="grid-3-col" style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:14,alignItems:'stretch'}}>
        ...
        <Card title={t("account.profile")}>
          <div style={{display:'flex',alignItems:'center',gap:14,marginBottom:14,paddingBottom:14,borderBottom:'1px solid #f1f3f7'}}>
            <div style={{position:'relative',flexShrink:0}}>
              ...
              <div style={{position:'absolute',bottom:-2,right:-2,width:20,height:20,borderRadius:'50%',background:'var(--sap-accent)',border:'2px solid #fff',display:'flex',alignItems:'center',justifyContent:'center'}}><span style={{fontSize:14,color:'#fff',fontWeight:800}}>+</span></div>
              <input type="file" accept="image/*" onChange={handleAvatarUpload} style={{display:'none'}}/>
              </label>
              {avatarUrl && <button onClick={removeAvatar} title={t('account.removePhoto')} style={{position:'absolute',top:-4,right:-4,width:18,height:18,borderRadius:'50%',background:'var(--sap-red-bright)',border:'2px solid #fff',display:'flex',alignItems:'center',justifyContent:'center',cursor:'pointer',fontSize:14,color:'#fff',fontWeight:800,lineHeight:1,padding:0,zIndex:2}}>✕</button>}
            </div>
            <div>
              <div style={{fontSize:16,fontWeight:800,color:'var(--sap-text-primary)'}}>{user.first_name||''} {user.last_name||''}</div>
              <div style={{fontSize:16,color:'var(--sap-text-muted)'}}>@{user.username}</div>
        ...
        <Card title={t("account.editProfile")}>
          ...
          <div style={{marginBottom:14}}>
            <label style={{fontSize:14,fontWeight:700,color:'var(--sap-text-muted)',textTransform:'uppercase',letterSpacing:.5,display:'block',marginBottom:4}}>{t("account.interests")} <span style={{fontWeight:400,textTransform:'none'}}>{t("account.interestsHelp")}</span></label>
            ...
            <div style={{fontSize:15,color:'var(--sap-text-muted)',marginTop:4}}>{t("account.interestsNote")}</div>
          </div>
        ...
        <Card title={t("account.kyc")} chip={kyc==='approved'?{t:t('account.verified'),c:'var(--sap-green)'}:kyc==='pending'?{t:t('account.underReview'),c:'var(--sap-amber)'}:kyc==='rejected'?{t:t('account.rejected'),c:'var(--sap-red)'}:{t:t('account.notStarted'),c:'var(--sap-text-muted)'}}>
          {kyc==='approved'?<div style={{textAlign:'center',padding:'20px 0'}}><div style={{fontSize:32,marginBottom:6}}>✅</div><div style={{fontSize:16,fontWeight:700,color:'var(--sap-green)'}}>{t("account.identityVerified")}</div><div style={{fontSize:14,color:'var(--sap-text-muted)'}}>{t("account.approvedWithdrawals")}</div></div>
          :kyc==='pending'?<div style={{textAlign:'center',padding:'20px 0'}}><div style={{fontSize:32,marginBottom:6}}>⏳</div><div style={{fontSize:16,fontWeight:700,color:'var(--sap-amber)'}}>{t("account.underReview").replace("⏳ ","")}</div><div style={{fontSize:14,color:'var(--sap-text-muted)'}}>{t("account.underReviewTime")}</div></div>
          :<>
            {kyc==='rejected'&&<div style={{padding:'8px 12px',background:'var(--sap-red-bg)',border:'1px solid #fecaca',borderRadius:8,marginBottom:10,fontSize:14,color:'var(--sap-red)',fontWeight:600}}>{t("account.rejectedResubmit")}</div>}
            <div>
              ...
              <div style={{marginBottom:12}}><label style={{fontSize:14,fontWeight:700,color:'var(--sap-text-muted)',textTransform:'uppercase',letterSpacing:.5,display:'block',marginBottom:4}}>{t("account.uploadId")}</label>
                <label style={{display:'block',width:'100%',padding:10,border:kycFile?'2px solid #0ea5e9':'2px dashed #d1d5db',borderRadius:10,fontSize:14,color:kycFile?'var(--sap-accent)':'var(--sap-text-muted)',background:'#fafbfc',cursor:'pointer',boxSizing:'border-box',textAlign:'center'}}>
  ...
  return (
    <div style={{background:'#fff',border:'1px solid #e8ecf2',borderRadius:12,overflow:'hidden',display:'flex',flexDirection:'column',boxShadow:'0 2px 8px rgba(0,0,0,.03)'}}>
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'12px 16px',background:'var(--sap-cobalt-deep)'}}>
        ...
        {chip&&<span style={{fontSize:14,fontWeight:700,padding:'2px 8px',borderRadius:4,background:chip.c+'20',color:chip.c}}>{chip.t}</span>}
      </div>
...
function Row({k, v, mono, last}) {
  return (
    <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'6px 0',borderBottom:last?'none':'1px solid #f5f6f8'}}>
      <span style={{fontSize:14,fontWeight:700,color:'var(--sap-text-muted)'}}>{k}</span>
      <span style={{fontSize:13,fontWeight:600,color:mono?'var(--sap-accent)':'var(--sap-text-primary)',...(mono?{fontFamily:'monospace'}:{})}}>{v}</span>
    </div>
  );
}
