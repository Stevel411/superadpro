...
export default function Wallet() {
  ...
  if (loading) return <AppLayout categoryBack={{ to: '/dashboard', label: 'Dashboard' }} title={t('wallet.title')}><LoadingSpinner /></AppLayout>;
  if (!data) return <AppLayout categoryBack={{ to: '/dashboard', label: 'Dashboard' }} title={t('wallet.title')}><div style={{ textAlign: 'center', padding: 80, color: 'var(--sap-text-faint)' }}>{t('wallet.unableToLoad')}</div></AppLayout>;
  if (showHelp) return <AppLayout categoryBack={{ to: '/dashboard', label: 'Dashboard' }} title={t('wallet.title')}><WalletHelp onBack={function() { setShowHelp(false); }}/></AppLayout>;
  ...
  return (
    <AppLayout categoryBack={{ to: '/dashboard', label: 'Dashboard' }} title={t('wallet.title')} subtitle={t('wallet.subtitle')}
    >
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 18, marginBottom: 18, alignItems: 'stretch' }}>
          ...
          <Card title={t("wallet.affiliateWallet")} dotColor="var(--sap-green)">
            {/* Universal daily watch-gate — one video unlocks affiliate withdrawals
                for the day. This is what supplies the campaign-tier owners' views. */}
    ...
    <div style={{ display:'flex', gap:16, marginBottom:18, alignItems:'stretch', flexWrap:'wrap' }}>
      <div style={{ flex:'1.6 1 340px', background:'linear-gradient(135deg,#172554,#1e3a8a)', borderRadius:16, padding:'22px 24px', color:'#fff', position:'relative', overflow:'hidden' }}>
        ...
        <div style={{ display:'flex', gap:10, flexWrap:'wrap' }}>
          ...
          {/* Universal daily watch-gate on the affiliate wallet — show for all. */}
          {d.watched_today === false ? (
