    ...
    >
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 18, marginBottom: 18, alignItems: 'stretch' }}>
          ...
          <Card title={t("wallet.affiliateWallet")} dotColor="var(--sap-green)">
            {/* Affiliate wallet is always withdrawable — no watch gate. */}
            <div style={{ padding:'10px 14px', background:'var(--sap-green-bg)', border:'1px solid #bbf7d0', borderRadius:10, marginBottom:14 }}>
              <div style={{ fontSize:16, fontWeight:700, color:'var(--sap-green-dark)', marginBottom:2 }}>{t('wallet.alwaysWithdrawable', { defaultValue: 'Always withdrawable' })}</div>
    ...
    <div style={{ display:'flex', gap:16, marginBottom:18, alignItems:'stretch', flexWrap:'wrap' }}>
      <div style={{ flex:'1.6 1 340px', background:'linear-gradient(135deg,#172554,#1e3a8a)', borderRadius:16, padding:'22px 24px', color:'#fff', position:'relative', overflow:'hidden' }}>
        ...
        <div style={{ display:'flex', gap:10, flexWrap:'wrap' }}>
          ...
          {/* Watch-gate chip applies to the CAMPAIGN wallet only (affiliate is
              always withdrawable), so only show it when there's a campaign
              balance to unlock. */}
          {(Number(d.campaign_balance) || 0) > 0 && (
            d.watched_today === false ? (
              <div style={{ background:'rgba(245,158,11,0.18)', border:'1px solid rgba(245,158,11,0.45)', borderRadius:9, padding:'7px 13px', fontSize:13, fontWeight:700, color:'#fde68a' }}>🔒 {t('wallet.watchToUnlockCampaign', { defaultValue: 'Watch to unlock campaign' })}</div>
            ) : (
