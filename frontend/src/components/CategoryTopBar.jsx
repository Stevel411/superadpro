...
export default function CategoryTopBar({ backTo = '/dashboard', backLabel = 'Dashboard', toolBrand = null }) {
  ...
  return (
    <div className="ctb">
      ...
      {toolBrand && (toolBrand.headline || toolBrand.title) && (
        <div className="mid">
          {toolBrand.headline ? (
            /* Big-statement variant (Steve's SuperLeads layout, 2 Jul 2026):
               the page headline lives in the topbar centre. Hidden <=900px —
               the page renders it in its hero card there instead. */
            <div className="mh">{toolBrand.headline[0]}<br/>{toolBrand.headline[1]}</div>
          ) : (
            <>
      ...
      <div className="rt">
        ...
        {menuOpen && (
          <div className="menu">
            ...
            <a href="/support-center"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 15a2 2 0 01-2 2H7l-4 4V5a2 2 0 012-2h14a2 2 0 012 2z"/></svg>Support</a>
            {user && user.is_admin && (
              <>
