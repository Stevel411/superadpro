...
Image generation via xAI's Grok Imagine model. Primary consumer is the
Brand Poster Generator (BPG) inside Creative Studio — generates complete
marketing posters from prompt templates (One Income Is Risky, Tired of
Scrolling, Pay It Forward, etc.) at launch quality.
...
Two endpoints used depending on whether we have a reference photo:
  ...
  - POST /v1/images/edits         (with reference photo of the member)
...
# + landscape) plus 3:4 (Brand Poster Generator default). xAI Imagine
# supports a specific set that does NOT include 4:5 — verified via live
# API test 12 May 2026 (returned 422 with the full list of valid values).
# Map 4:5 → 3:4 (closest portrait, ~5% squarer).
_XAI_ASPECT_MAP = {
    "1:1":  "1:1",
    "3:4":  "3:4",   # BPG default — Instagram portrait poster
    "4:3":  "4:3",
