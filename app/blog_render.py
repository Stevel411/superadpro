...
def _article_markup(ctx):
    ...
    return (
        ...
        f'<div class="f"><form method="post" action="/sites/{escape(ctx.slug)}/subscribe" style="display:contents"><input name="email" type="email" required placeholder="you@email.com"><button type="submit">Join</button></form></div></div>{_comments_html(ctx)}</article>'
    )
...
*{margin:0;padding:0;box-sizing:border-box}html{background:var(--navy)}body{font-family:'Inter',sans-serif;color:var(--ink);background:var(--bg)}
...
.layout{display:grid;grid-template-columns:minmax(0,1fr) 300px;gap:clamp(32px,4vw,56px);padding:50px 0 64px;align-items:start}
.post{padding-bottom:38px;margin-bottom:38px;border-bottom:1px solid var(--line)}.post:last-child{padding-bottom:0;margin-bottom:0;border-bottom:none}
...
.post .thumb{aspect-ratio:16/9;border-radius:10px;margin-bottom:18px;background-size:cover;background-position:center}
.side{position:sticky;top:24px;align-self:start}.side .widget{background:#fff;border:1px solid var(--line);border-radius:14px;padding:24px;margin-bottom:24px}
...
.side .widget.subbox{background:var(--navy);color:#fff}.subbox h4{color:#fff;border-color:#fff}.subbox p{font-size:13.5px;color:#c3cbdb;margin-bottom:14px}
...
def render_cs_feed(ctx):
    ...
    _popw=f'<div class="widget"><h4>Popular</h4>{popular}</div>' if popular else ""
    _topw=f'<div class="widget"><h4>Topics</h4><div class="tags">{topics}</div></div>' if topics else ""
...
def render_bn_feed(ctx):
    ...
    _tags_tile=(f'<div class="tile t-tags"><h3>Topics</h3><div class="ts">{topics}</div></div>' if topics else "")
    tiles+=(_tags_tile
...
def render_cn_feed(ctx):
    ...
    return (f"{head}<style>{_CN_CSS}{_palette_css(ctx)}</style></head><body>{_cn_header(ctx)}"
        ...
        f'<div class="f"><form method="post" action="/sites/{escape(ctx.slug)}/subscribe" style="display:contents"><input name="email" type="email" required placeholder="you@email.com"><button type="submit">Join</button></form></div></div></div>{_cn_footer(ctx)}</body></html>')
...
def render_gl_feed(ctx):
    ...
    return (f"{head}<style>{_GL_CSS}{_palette_css(ctx)}</style></head><body>{_gl_header(ctx)}"
        ...
        f'<div class="f"><form method="post" action="/sites/{escape(ctx.slug)}/subscribe" style="display:contents"><input name="email" type="email" required placeholder="you@email.com"><button type="submit">Join</button></form></div></div></div>{_gl_footer(ctx)}</body></html>')
