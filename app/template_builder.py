...
def _lead_capture(username):
    ...
    els.append({'id': _uid(), 'type': 'heading', 'x': _centre(900), 'y': y, 'w': 900, 'h': 55, 'txt': 'Start Building Your Online Income Today', 's': {'fontFamily': 'Sora,sans-serif', 'fontWeight': '900', 'fontSize': '42px', 'color': '#fff', 'textAlign': 'center'}})
    y += 70

    els.append({'id': _uid(), 'type': 'text', 'x': _centre(700), 'y': y, 'w': 700, 'h': 50, 'txt': 'Get instant access to our proven system — professional marketing funnels, AI-powered tools, and step-by-step training. Enter your email below to get started.', 's': {'fontFamily': 'Outfit,sans-serif', 'fontSize': '16px', 'color': '#94a3b8', 'textAlign': 'center', 'lineHeight': '1.8'}})
    y += 70
    ...
    for i, (icon, title, desc) in enumerate(features):
        fx = 50 + i * 350
        els.append({'id': _uid(), 'type': 'icontext', 'x': fx, 'y': y, 'w': 320, 'h': 80,
    ...
    for i, (name, role, text) in enumerate(testimonials):
        tx = 50 + i * 350
        els.append({'id': _uid(), 'type': 'testimonial', 'x': tx, 'y': y, 'w': 320, 'h': 140,
    ...
    # CTA
    els.append({'id': _uid(), 'type': 'heading', 'x': _centre(800), 'y': y, 'w': 800, 'h': 45, 'txt': 'Ready to Start Earning?', 's': {'fontFamily': 'Sora,sans-serif', 'fontWeight': '800', 'fontSize': '28px', 'color': '#fff', 'textAlign': 'center'}})
    y += 55
    els.append({'id': _uid(), 'type': 'text', 'x': _centre(600), 'y': y, 'w': 600, 'h': 30, 'txt': 'Join thousands of members building real income online — your spot is waiting.', 's': {'fontSize': '15px', 'color': '#64748b', 'textAlign': 'center'}})
    y += 45
...
def _video_sales(username):
    ...
    els.append({'id': _uid(), 'type': 'heading', 'x': _centre(950), 'y': y, 'w': 950, 'h': 65, 'txt': 'How Everyday People Are Earning $500–$5,000/Month From Their Phone', 's': {'fontFamily': 'Sora,sans-serif', 'fontWeight': '900', 'fontSize': '38px', 'color': '#fff', 'textAlign': 'center'}})
    y += 80

    els.append({'id': _uid(), 'type': 'text', 'x': _centre(650), 'y': y, 'w': 650, 'h': 40, 'txt': 'Watch the 3-minute video below to discover the simple system that\'s changing lives — no experience needed.', 's': {'fontSize': '16px', 'color': '#94a3b8', 'textAlign': 'center', 'lineHeight': '1.7'}})
    y += 60
    ...
    # Testimonial
    els.append({'id': _uid(), 'type': 'testimonial', 'x': _centre(700), 'y': y, 'w': 700, 'h': 130,
...
def _product_offer(username):
    ...
    els.append({'id': _uid(), 'type': 'heading', 'x': _centre(900), 'y': y, 'w': 900, 'h': 55, 'txt': 'The Complete Online Business Toolkit', 's': {'fontFamily': 'Sora,sans-serif', 'fontWeight': '900', 'fontSize': '40px', 'color': '#fff', 'textAlign': 'center'}})
    y += 70

    els.append({'id': _uid(), 'type': 'text', 'x': _centre(700), 'y': y, 'w': 700, 'h': 40, 'txt': 'Everything you need to launch, grow and scale your online business — in one powerful platform.', 's': {'fontSize': '16px', 'color': '#94a3b8', 'textAlign': 'center', 'lineHeight': '1.8'}})
    y += 70
    ...
    # Guarantee
    els.append({'id': _uid(), 'type': 'text', 'x': _centre(500), 'y': y, 'w': 500, 'h': 30, 'txt': '🛡️ 30-Day Money Back Guarantee — Zero Risk', 's': {'fontSize': '14px', 'color': '#64748b', 'textAlign': 'center'}})
...
def _business_opportunity(username):
    ...
    els.append({'id': _uid(), 'type': 'heading', 'x': _centre(950), 'y': y, 'w': 950, 'h': 65, 'txt': 'Tired of Working Hard for Someone Else\'s Dream?', 's': {'fontFamily': 'Sora,sans-serif', 'fontWeight': '900', 'fontSize': '40px', 'color': '#fff', 'textAlign': 'center'}})
    y += 80

    els.append({'id': _uid(), 'type': 'text', 'x': _centre(700), 'y': y, 'w': 700, 'h': 50, 'txt': 'Discover how thousands of ordinary people are building extraordinary income from home — with a simple, proven system that anyone can follow.', 's': {'fontSize': '16px', 'color': '#94a3b8', 'textAlign': 'center', 'lineHeight': '1.8'}})
    y += 70
    ...
    for i, (num, title, desc) in enumerate(steps):
        sx = 50 + i * 350
        els.append({'id': _uid(), 'type': 'icontext', 'x': sx, 'y': y, 'w': 320, 'h': 90,
    ...
    # Testimonials
    els.append({'id': _uid(), 'type': 'testimonial', 'x': 50, 'y': y, 'w': 500, 'h': 130,
    ...
    els.append({'id': _uid(), 'type': 'testimonial', 'x': 570, 'y': y, 'w': 500, 'h': 130,
...
def _webinar_registration(username):
    ...
    els.append({'id': _uid(), 'type': 'heading', 'x': _centre(900), 'y': y, 'w': 900, 'h': 65, 'txt': 'How to Build a $5,000/Month Online Business in 90 Days', 's': {'fontFamily': 'Sora,sans-serif', 'fontWeight': '900', 'fontSize': '38px', 'color': '#fff', 'textAlign': 'center'}})
    y += 80

    els.append({'id': _uid(), 'type': 'text', 'x': _centre(650), 'y': y, 'w': 650, 'h': 50, 'txt': 'Join our FREE 60-minute masterclass and learn the exact 3-step system used by 10,000+ members to generate consistent online income.', 's': {'fontSize': '16px', 'color': '#94a3b8', 'textAlign': 'center', 'lineHeight': '1.8'}})
    y += 70
    ...
    # What you'll learn
    els.append({'id': _uid(), 'type': 'heading', 'x': _centre(600), 'y': y, 'w': 600, 'h': 40, 'txt': 'In This Free Training You\'ll Discover:', 's': {'fontFamily': 'Sora,sans-serif', 'fontWeight': '800', 'fontSize': '24px', 'color': '#fff', 'textAlign': 'center'}})
    y += 55
    ...
    for i, (icon, title, desc) in enumerate(learns):
        lx = 50 + i * 350
        els.append({'id': _uid(), 'type': 'icontext', 'x': lx, 'y': y, 'w': 320, 'h': 80,
            'txt': f'<div style="display:flex;gap:16px;align-items:flex-start"><div style="font-size:28px;flex-shrink:0;width:40px;text-align:center">{icon}</div><div><div style="font-family:Sora,sans-serif;font-weight:700;font-size:15px;color:#fff;margin-bottom:4px">{title}</div><div style="font-size:13px;color:#94a3b8;line-height:1.6">{desc}</div></div></div>',
            's': {}})
...
def _coaching_program(username):
    ...
    els.append({'id': _uid(), 'type': 'heading', 'x': _centre(800), 'y': y, 'w': 800, 'h': 55, 'txt': 'Transform Your Life With Expert Coaching', 's': {'fontFamily': 'Sora,sans-serif', 'fontWeight': '900', 'fontSize': '40px', 'color': '#fff', 'textAlign': 'center'}})
    y += 70

    els.append({'id': _uid(), 'type': 'text', 'x': _centre(650), 'y': y, 'w': 650, 'h': 50, 'txt': 'Personalised 1-on-1 coaching to help you build a thriving online business. Get clarity, accountability, and a proven roadmap to your income goals.', 's': {'fontSize': '16px', 'color': '#94a3b8', 'textAlign': 'center', 'lineHeight': '1.8'}})
    y += 70
    ...
    for i, (icon, title, desc) in enumerate(includes):
        ix = 50 + i * 350
        els.append({'id': _uid(), 'type': 'review', 'x': ix, 'y': y, 'w': 320, 'h': 110,
    ...
    # Testimonials
    els.append({'id': _uid(), 'type': 'heading', 'x': _centre(500), 'y': y, 'w': 500, 'h': 40, 'txt': 'What Our Clients Say', 's': {'fontFamily': 'Sora,sans-serif', 'fontWeight': '800', 'fontSize': '24px', 'color': '#fff', 'textAlign': 'center'}})
    y += 55

    els.append({'id': _uid(), 'type': 'testimonial', 'x': 50, 'y': y, 'w': 500, 'h': 120,
    ...
    els.append({'id': _uid(), 'type': 'testimonial', 'x': 570, 'y': y, 'w': 500, 'h': 120,
...
def _digital_product(username):
    ...
    els.append({'id': _uid(), 'type': 'heading', 'x': _centre(900), 'y': y, 'w': 900, 'h': 55, 'txt': 'The Ultimate Guide to Online Income', 's': {'fontFamily': 'Sora,sans-serif', 'fontWeight': '900', 'fontSize': '40px', 'color': '#fff', 'textAlign': 'center'}})
    y += 70

    els.append({'id': _uid(), 'type': 'text', 'x': _centre(700), 'y': y, 'w': 700, 'h': 50, 'txt': '97 pages of actionable strategies, templates and blueprints to build your first $1,000/month online — even if you\'re starting from zero.', 's': {'fontSize': '16px', 'color': '#94a3b8', 'textAlign': 'center', 'lineHeight': '1.8'}})
    y += 80

    # What's inside
    els.append({'id': _uid(), 'type': 'heading', 'x': _centre(400), 'y': y, 'w': 400, 'h': 35, 'txt': 'What\'s Inside:', 's': {'fontFamily': 'Sora,sans-serif', 'fontWeight': '800', 'fontSize': '22px', 'color': '#14b8a6', 'textAlign': 'center'}})
    y += 50
...
def _affiliate_funnel(username):
    ...
    els.append({'id': _uid(), 'type': 'heading', 'x': _centre(900), 'y': y, 'w': 900, 'h': 65, 'txt': 'Turn Your Network Into a Money-Making Machine', 's': {'fontFamily': 'Sora,sans-serif', 'fontWeight': '900', 'fontSize': '40px', 'color': '#fff', 'textAlign': 'center'}})
    y += 80

    els.append({'id': _uid(), 'type': 'text', 'x': _centre(700), 'y': y, 'w': 700, 'h': 50, 'txt': 'Share your unique link. Earn 50% commission on every referral. Get paid monthly — it\'s that simple. No products to create, no orders to fulfil.', 's': {'fontSize': '16px', 'color': '#94a3b8', 'textAlign': 'center', 'lineHeight': '1.8'}})
    y += 70

    # Earnings potential
    els.append({'id': _uid(), 'type': 'heading', 'x': _centre(500), 'y': y, 'w': 500, 'h': 35, 'txt': 'Your Earning Potential:', 's': {'fontFamily': 'Sora,sans-serif', 'fontWeight': '800', 'fontSize': '22px', 'color': '#fff', 'textAlign': 'center'}})
    y += 50
    ...
    for i, (icon, title, desc) in enumerate(steps):
        sx = 50 + i * 350
        els.append({'id': _uid(), 'type': 'icontext', 'x': sx, 'y': y, 'w': 320, 'h': 80,
            'txt': f'<div style="display:flex;gap:16px;align-items:flex-start"><div style="font-size:28px;flex-shrink:0;width:40px;text-align:center">{icon}</div><div><div style="font-family:Sora,sans-serif;font-weight:700;font-size:15px;color:#fff;margin-bottom:4px">{title}</div><div style="font-size:13px;color:#94a3b8;line-height:1.6">{desc}</div></div></div>',
            's': {}})
